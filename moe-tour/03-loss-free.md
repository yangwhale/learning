...
Loss-Free 解决了 Aux Loss 的参数冲突问题，但留下了两个开放问题：

1. **均匀分布是最优目标吗？** 所有 expert 负载相同真的最好吗？有些 token 更难处理，是否应该给它们更多 expert？（→ #4 动态激活）
2. **偏置 b 的自由度怎么利用？** 全体 b_i 加常数不影响排序，这个多余的自由度能用来做什么？（→ #4 也会涉及）
3. **能否完全消除 capacity_factor？** Loss-Free 的统计均衡在单 batch 内仍有波动，能否做到精确均衡？（→ #6 Quantile Balancing）
