...
> **后见之明**：DeepSeek V4 的 Hash Routing（番外篇解读）可以看作 Shared Expert 思想在时间轴上的延伸。Shared Expert 说"有些知识每个 token 都需要"，Hash Routing 说"有些层每个 token 的路由都是确定性的"。两者都承认同一个事实——**并非所有计算都需要动态决策**。V4 最终的架构是 first_k_hash（浅层确定性路由 MoE）+ 中深层 Router MoE + Shared Expert，三种机制各自覆盖非均匀性的不同维度。
...
本文的"非均匀"思想在后续系列中从多个维度展开：
- **第 8 篇 MQB**：从"全局均匀不一定对"推进到"序列内部也不应该均匀分配"——序列级均衡是另一种非均匀性
- **番外篇 Hash Routing**：从"不是所有 expert 都同等重要"推进到"不是所有层都需要动态路由"——层级异质性
- **第 9 篇门控归一化**：从"不需要 Softmax"推进到"各种门控效果差不多"——真正重要的不是门控函数，而是均衡算法
