...
这个观点后来被 DeepSeek-V3 的 Sigmoid gating 验证——非归一化的 Router 确实效果更好。
...
**MoE 不是一种新架构，而是 Dense 模型在计算约束下的最优退化形式。**
...
第一篇建立了 MoE 的几何基础，但留下了一个核心问题没有讨论：

> top-k 选择意味着不同 expert 被激活的频率不同。如果某些 expert 长期不被激活，它们的参数就无法得到训练（梯度为零），最终变成"死 expert"。反过来，如果少数 expert 被过度激活，又会导致计算负载不均衡。

这就是**负载均衡问题**——整个系列最核心的主题，从第 2 篇开始展开：

- #2 Aux Loss：最简单的均衡方案（加惩罚项）
- #3 Loss-Free：不加惩罚项的均衡方案（DeepSeek）
- #6 Quantile Balancing：无超参数的最优均衡方案（苏剑林自己提出）
...
**Dense FFN 天然是 n 个 expert 之和；MoE 就是在正交近似下按模长选 top-k 来逼近这个和；Router 的本质是"模长预测器"。这个几何框架最终通向 Quantile Balancing，在 TPU 的静态编译约束下释放出最大价值。**
