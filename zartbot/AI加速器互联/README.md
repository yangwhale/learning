...
```svg
![Direct Dispatch Kernel — Warp 角色分工](assets/deepep-01.png)
```
...
```svg
![Combine 主 Kernel — 三分支决策](assets/deepep-02.png)
```
...
```svg
![Dispatch → Expert → Combine 完整 Kernel 流水线](assets/deepep-03.png)
```
...
```svg
![Hybrid 拓扑：world = scaleout × scaleup](assets/deepep-04.png)
```
...
```svg
![Hybrid Dispatch — 两跳数据流](assets/deepep-05.png)
```
...
```svg
![Hybrid Combine — 反向两跳 + 通信计算重叠](assets/deepep-06.png)
```
...
```svg
![一张对称 VA，两条搬运腿](assets/deepep-07.png)
```
...
```svg
![对称 VA 的三步拼装](assets/deepep-08.png)
```
...
```svg
![LLA2A：一次 16B 原子写 = 数据 + 通知](assets/deepep-09.png)
```
...
```svg
![GIN：GPU 发起的跨机通信，两种后端](assets/deepep-10.png)
```
...
```svg
![Hybrid AlltoAll：一个 kernel，两条腿并行](assets/deepep-11.png)
```
