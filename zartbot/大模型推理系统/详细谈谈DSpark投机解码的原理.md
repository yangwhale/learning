...
基本设定
*目标模型*: 在给定前文 的情况下, 对下一个 token 的输出概率分布为. 我们简写为.

*草稿模型*: 在同样的前文下, 输出概率分布为. 我们简写为.

我们的目标是, 以某种方式从 中采样一个 token, 但要尽量避免直接计算和调用昂贵的.这背后的数学原理是

拒绝采样(Rejection Sampling)
假设我们想从一个目标分布 中采样, 但直接采样很困难. 不过, 我们可以很容易地从另一个提议分布 中采样, 并且存在一个常数, 使得对于所有的, 都有.拒绝采样的步骤如下:
...
可以证明, 通过这个过程接受的样本, 其分布*恰好是*目标分布.

在投机解码中, 我们将这个思想应用到序列生成上. 假设草稿模型生成了 个 token的序列. 目标模型需要对它们进行验证. 验证过程从左到右 (对于):

![图片](assets/1708dfc03492.jpg)

草稿模型在第 步生成了 token, 其概率为
...
为什么这个接受概率是?
标准的拒绝采样**前提条件**是找到常数, 使得对所有 都有：

即 是 的一个**上包络**.关键在于：**在候选点 处, 接受的概率与 成正比**.

![图片](assets/ffc92bf1cc43.jpg)

, 所以**q 越接近 p, M 越小, 效率越高**

这可以看作是拒绝采样的一个变种, 在论文中普遍也被称为*投机采样(Speculative Sampling)*. 在实践中, 我们通常假设草稿模型的分布不会与目标模型差得太离谱. 如果碰巧在某个点, 我们就直接以概率1接受它, 这种情况非常有利. 如果, 那么接受的概率就是比率. 这两种情况合并起来, 就是.

恢复目标模型分布证明
给定离散分布, 和一个从 中采样的草稿样本, 令 为最终得到的样本. 为真的情况有两种: 我们要么采样得到 并接受它, 要么在 (无论其值为何)被拒绝后重新采样得到它. 因此:
...
我们已经恢复了期望的目标分布.
渣注
看上去证明很复杂, 我来说人话, 大致的证明思路是:  任何一个token 最终被我们选中, 只有两条路可以走. 我们把这两条路的概率加起来, 看看最终是不是等于“教授”本来想选 的概率.
...
两者相乘, 这一路径的总概率是, 化简后就是. 这代表了学生和教授的 "共识" 部分.

**情况**: 学生的草稿刚好写了, 而且教授也认可了.

**概率计算**:

**路径二: "另请高明" (草稿被拒绝, 重新采样)**

首先, 我们需要知道 "发生拒绝" 这件事本身的总概率. 证明中通过一系列计算得出, 这个总概率等于所有. 这部分代表了教授的想法超出学生认知的所有 "概率溢出" 的总量.

然后, 在发生拒绝后, 教授会从一个 "修正列表" 中挑选. 被挑中的条件概率就是

两者相乘,`(拒绝总概率) * (拒绝后选中x的条件概率)`, 分子分母正好抵消, 最终这条路径的总概率就是. 这精确地代表了 这个 token 上, 教授比学生更偏好的那部分 "独特想法".

**情况**: 学生写的任何草稿被教授拒绝了, 然后在补救环节, 教授亲自挑选出了.
...
**结论**: 账平了! 无论通过哪条路径, 最终选中 的总概率不多不少, 正好就是教授最初想选 的概率. 证明完毕.
...
**接受率** 衡量"草稿有多靠谱". 形式上,, 即目标分布 与草稿分布 之间全变差距离(Total Variation Distance)的互补. 直觉上, 等于两个分布的"重叠面积", 越大说明草稿越接近目标.

**成本比** 衡量"起草有多便宜"., 即草稿模型单步推理时间与目标模型的比值. 理想情况下, 但能保证合理接受率的草稿模型通常不会太小, 实践中 在0.05-0.2之间.

**草稿长度** 衡量"每轮猜多少个". 太小则投机收益有限; 太大则后面token的接受概率指数下降(连续 次全部猜对的概率为), 反而浪费起草成本.
加速比的精确公式
三者共同决定了端到端加速比:

**分子** 是每轮期望接受的token数. 这是一个截断几何级数, 完全由草稿质量 决定.

**分母** 是每轮的时间成本, 是起草 个token的相对耗时, 1 是验证一次的耗时.
...
在第次解码迭代中, 小模型和大模型各自以部分生成的文本为输入, 生成在整个词汇表上的概率分布和. 然后, 下一个token和分别从这些分布中采样:

根据策略函数的返回值(布尔值), 决定使用哪个模型的预测:

目标是设计一个轻量级策略, 使得在最小端到端延迟下获得高质量文本生成, 仅在必要时调用大模型.

Fallback策略: 小模型知道何时停止预测
策略的第一个原则是小模型应能判断何时将控制权交给大模型. 每当小模型对其预测缺乏信心时, 应让大模型接管. 作者发现一个基于最大预测概率的简单策略就足够有效, 即. 如果最大预测概率低于某个阈值, 则认为小模型的预测不够自信, 回退到大模型生成下一个token.

**Fallback策略**: 如果, 则回退到大模型, 令.
...
对于某个距离度量, 找到满足以下条件的最小解码步骤:

如果这样的存在, 则认为小模型在位置的预测不准确, 回滚从到的所有预测(因为它们都依赖于错误的预测), 并用大模型的预测替换.

**Rollback策略**: 如果存在最小的使得, 则回滚预测并令.
...
**目标函数**: 接收层选择向量 作为输入, 返回在开发集上每个 verified token 的平均推理时间. 这是一个综合度量, 包含了 drafting 和 verifying 两个阶段.

**优化问题**:

其中 是总层数.

然后采用贝叶斯优化算法迭代地基于代理模型 (高斯过程) 和采集函数选择新的输入 进行评估. 采集函数平衡了探索 (在不确定区域测试) 和利用 (在预期良好区域测试). 优化持续直到达到预定迭代次数. 搜索得到的 在模型级别固定, 不再更新. 当目标任务差异很大时, 可以进行任务级优化. 此外, 跳层方案可以与**量化**和**稀疏化**结合, 进一步降低资源消耗.
...
 是第 次 drafting 阶段的接受率

当 时,

每次验证阶段后更新. 该更新规则确保接受率维持在目标接受率 附近.
...
Expand阶段
一个 token 的全局接受率是从根节点到 路径上所有 token 接受率的乘积. 因此将其定义为价值:

其中, 表示在草稿树中从根节点到节点 的路径, 表示节点 的接受率, 而 表示来自草稿模型的 的置信度分数. 置信度分数与接受率强烈正相关. 因此利用这种关系来近似这个价值.从价值更高的 token 开始的分支更有可能被接受. 因此, 选择最后一层中价值最高的 top-k 个节点作为草稿模型的输入, 并根据输出扩展草稿树.
...
Meta的论文《Better & Faster Large Language Models via Multi-token Prediction》[12] 提出了一个问题:**为什么不在预训练阶段就赋予模型投机能力？** 通过添加 个独立输出头同时预测未来 个 token, 多 token 预测不仅提升了模型在下游任务(尤其是代码生成)上的质量, 还让这些输出头在推理时直接充当投机解码的草稿, 实现了训练增强与推理加速的统一.

![图片](assets/283c3f40c6a9.png)

#### 1.4.2 DeepSeek-V3 MTP

Meta MTP使用**并行**的独立输出头预测个额外token; DeepSeek-V3则是**顺序**预测, 且保持每个预测深度的完整因果链.
...
其中为权重因子 (前10T tokens设为0.3, 后4.8T设为0.1).

DeepSeek-V3的MTP在每个深度都保持了完整的因果链, 而非像 Meta MTP 那样用独立头并行预测. 这意味着第层的预测依赖于第层的输出, 形成了自然的信息传递链路. DeepSeek-V3 MTP标志着投机解码从"后训练附加"到"预训练内置"的范式转变. 它证明了: 将多步预测能力融入预训练, 既能提升模型本身的质量(训练信号密集化), 又能获得推理加速的"免费午餐".

### 1.5 Dflash

前四个阶段的所有方法, 无论多么精巧, 都受制于同一个根本性约束:**自回归串行起草**. 即使EAGLE的草稿模型仅有1层, 生成 个token仍需要7次串行前向传播. 起草延迟 随草稿长度线性增长, 这迫使设计者在两个方向做出妥协: 要么限制草稿长度(通常~8), 要么限制模型深度(通常1层). 无论怎么选择, 的线性瓶颈都像一堵墙, 堵死了投机解码进一步突破的空间.
...
即一个锚点token(上一轮验证确认的最后一个token)加上 个[MASK]占位符. 块内所有位置使用**双向注意力**(而非因果注意力), 每个位置都能看到其他所有位置, 这正是扩散模型的天然特性. 单次前向传播后, 模型一次性输出所有 个位置的预测.

关键指标:**延迟与 无关**. 生成16个token和生成4个token的延迟几乎相同. 这是因为现代GPU的并行计算能力可以轻松处理多个位置的同时计算, 真正的瓶颈在于权重加载(访存受限)而非计算本身.
...
原本注意力机制的Key-Value空间维度为(草稿序列长度), KV注入后扩展为(目标特征长度 + 草稿序列长度):
...
KV注入通过在信息几何意义上将草稿分布 "拉向"目标分布, 减小两者在统计流形上的距离, 从而提升接受率.

接下来是并行扩散草稿阶段(Diffusion Drafting). 传统自回归起草需要 次串行前向传播(每次生成1个token), 而DFlash只需1次前向传播即可生成 个token:
...
`[:, 1 - block_size :, :]`: 只取最后 个位置的输出(第1个位置是锚点, 不需要预测)
...
标准 Speculative Sampling 要求草稿模型提供每个位置的条件概率:. 但 DFlash 的草稿模型是**块扩散模型**, 它通过单次前向传播**同时生成**所有位置的 token, 产出的是一个联合分布, 无法自然分解为逐位置的条件概率. 因此 DFlash 采用了更简洁的**"采样后比较"** 策略:
...
位置加权损失
在投机解码中, 前面位置的错误代价远大于后面位置, 位置 的拒绝会导致所有后续token被丢弃. DFlash使用指数衰减的损失权重来反映这种不对称性:
...
**自回归起草** (EAGLE-3):, 因模型容量限制快速饱和

**扩散起草** (DFlash):, 随模型深度有效增长

DFlash的关键优势: 由于 对 不敏感, 可以同时增大(更长草稿)和增加模型深度(更高接受率), 在"起草质量-起草延迟"的帕累托前沿上完全占优.
...
Proposition 2
证明贪心=最优: 由于因子化分布的乘积结构, 祖先概率 > 后代概率 (), 所以按概率排序选top-B时, 祖先一定先于后代被选中, 前缀闭合性自动满足.
...
对应论文Proposition 3: 此算法保证返回目标 下的精确最优树, 复杂度.
...
其中 为从根到节点 的路径, 为融合隐状态特征. 这个分解**镜像了**目标模型的自回归分解:

为什么因果性至关重要?
没有因果条件化时(如DFlash), 树构造按照伪分布排序:

其中 为位置 处的分支无关分布. 这意味着单独合理但相互不一致的 token 组合会被优先选择. 例如前缀为 "We" 时, 扩散头可能选出 " given told that" , "given" 和 "told" 单独看合理, 但不能相邻出现.
...
Tree Verification
接受规则基于投机解码的拒绝采样:
$$\alpha_t = \min\left(1, \frac{p(y_t | x, y_{<t})}{q(y_t |="" x,="" y_{<t})}\right)="" $$=""代码实现如下
...
令 表示每个周期接受的 token 数量, 令 和 分别表示起草和验证过程的 Wall clock 时间. 平均每个生成 token 的延迟为:

因此, 提升加速比可以归结为三个杠杆: 降低 (更快地起草), 提高 (更好地起草), 或者降低有效的 (更智能地验证). 作者明确指出了三个努力方向:
...
DFlash一个并行起草模型在一次前向传播中产生所有 个草稿的logits, 因此每个预测都不能以块内其他位置已采样的 token 为条件. 当上下文允许多种合理的续写时, 例如"of course"和"no problem", 并行起草模型可能会产生不连贯的组合, 如"of problem"或"no course", 因为每个位置都是对所有可能的前置 token 进行边缘化, 而不是以实际采样的那一个为条件. 因此, 接受率会沿着草稿块迅速衰减, 浪费了起草和验证的计算资源. 因此, 作者采用一种半自回归结构, 将草稿生成分为两个阶段:

![图片](assets/3939c058c923.jpg)
并行阶段 (Parallel stage)
一个并行主干网络(在我们的实例中是DFlash)在整个块上运行一次前向传播, 产生隐藏状态 和基础logits. 作者只对原始的DFlash主干做了一个微小的修改: 不输入一个锚点 token 加上 个掩码 token 并只预测掩码位置, 而是将锚点本身视为第一个预测位置, 因此 个输入 token (锚点 + 个掩码)产生 个草稿logits. 这在保持相似草稿质量的同时, 减少了起草的计算量.
...
 表示上一轮验证周期中目标模型生成的锚点 token

 是并行主干在位置 产生的基础logit向量

 是词汇表.

在推理时, 串行模块根据 从左到右进行采样. 由于这个采样过程本质上是串行的, 该模块的计算必须是轻量级的 (), 以便总的起草延迟仍然由并行阶段主导. 我们在下面描述该串行模块的两种实现.
马尔可夫头 (Markov head)
最简单的实现将 的依赖限制在紧邻的前一个 token 上, 将其简化为一阶转移. 原则上这是一个完整的 矩阵; 用一个低秩分解来近似它:, 其中 且. 给定前一个 token, 位置 的转移偏差为:
...
```
RNN头 (RNN head)
马尔可夫头在一步之后就没有记忆了, 位置 无法访问 之前的 token . RNN头通过维持一个循环状态 来放宽这个限制, 该状态累积了块内的完整前缀历史. 在每一步, 该模块将当前状态, 前一个 token 的嵌入, 以及主干网络的隐藏状态 拼接成一个输入向量, 然后应用一次门控更新:

其中 由一个单一的线性投影联合参数化, 该投影被分割为门、候选和输出三个部分. 状态 初始化为零.
...
而低秩分解 是一个经典的数学技巧. 它假设完整的转移矩阵 是低秩的, 即 token 间的转移关系可以由一个更小的"概念"空间(维度为)来描述. 将一个 token 映射到这个概念空间, 再从这个概念空间映射回完整的logits空间. 这本质上是一种压缩表示, 大大降低了模型的参数量和计算复杂度.
...
置信度头为每个草稿位置 输出一个标量估计值. 至关重要的是, 建模的是在块内所有先前 token 都已被接受的条件下, 位置 的草稿 token 能够通过目标模型验证的**条件概率**. 该架构采用一个轻量级的线性投影后接一个sigmoid函数:

其中 是主干网络的Hidden State, 是来自前一个草稿 token 的马尔可夫嵌入. 使用每一步的解析接受率 来监督. 这个率由草稿分布 和目标分布 之间的总变差距离决定:
...
为了解决这个问题, 引入了**序列化温度缩放 (Sequential Temperature Scaling, STS)**. 因为每个 建模的是一个条件概率, 链式法则决定了一个草稿前缀被接受的联合概率可以分解为累积乘积. 使用一个留出的验证集, STS从左到右依次校准这个联合概率. 具体来说, 在每个位置, 作者进行一个简单的1D网格搜索来找到最优的温度标量, 以最小化累积乘积的期望校准误差(ECE), 同时保持所有先前位置已校准的分数固定. 关键的是, 温度缩放是一个保序变换: 它在纠正预测概率以匹配经验接受率的同时, 不会扰乱置信度头学到的相对草稿 token 排名.
...
为了解决这个问题, 我们将验证长度选择问题形式化为一个**全局吞吐量最大化问题 (算法1)**. 考虑一个包含 个活动请求的批次. 对于请求, 令 为每个位置的置信度估计, 令 表示被调度的验证长度. 由于投机解码只动态地接受连续前缀的草稿 token , 一个在位置 的 token 的存活概率是累积乘积.

![图片](assets/e3aa6f86b26b.png)

在单次验证步骤中, 发送到目标模型的总批次大小(以 token 计)为, 预期的成功接受 token 数为. 令 表示引擎对于给定前向传播批次大小 的吞吐量, 以每秒步数(steps per second)计. 关键是, 这个容量曲线在引擎初始化时分析一次, 并存储为一个轻量级的成本表. 调度器旨在通过动态选择验证长度 来最大化预期的系统范围 token 吞吐量.
...
调度器的目标函数
这是一个在组合优化中常见的目标形式. 它不是简单地最大化收益()或最小化成本(与 成正比), 而是最大化它们的比率, 即效率.
贪心算法的正确性
论文指出, "单调性确保了按 全局排序...是有效的". 我们可以更深入地分析这一点. 每次贪心选择, 都是选择使"边际收益/边际成本"最大的那一项. 这里的边际收益是, 边际成本是使总批次大小B增加1所带来的吞吐量下降 (即). 算法1的简化贪心策略之所以有效, 是因为它假设了所有边际成本(增加一个token)是相同的, 于是问题简化为最大化边际收益, 即贪心地选择 最大的项.
因果性与鞅理论
"非预见性"原则在数学上与**鞅(Martingale)** 的概念密切相关. 一个随机过程是鞅, 如果其在未来的期望值等于现在的观测值. 投机解码的无损性要求最终采样的分布不受未来信息的影响. 如果调度决策依赖了, 那么对 的采样过程就不是一个纯粹的、基于 的过程, 其条件期望会发生改变, 从而破坏了鞅的性质, 导致分布偏差. 算法1中的`break`确保了决策只依赖于"过滤(filtration)"中到当前为止的信息, 维持了过程的正确性.

附录有一个因果性反例详解, 解释为什么不能去掉`break`.

### 2.3 训练

在训练期间, 作者从每个目标序列中随机采样多个锚点位置, 以形成 token 块作为训练数据. 目标模型在整个训练过程中保持冻结; 草稿模型共享其嵌入层和语言建模头并保持它们冻结, 只更新主干起草模型、串行模块和置信度头.

训练目标由三项组成: 一个交叉熵损失, 一个分布匹配损失, 和一个置信度损失. 这三者都通过 进行位置加权, 这强调了在基于前缀的验证下对预期接受长度贡献更多的块中较早的位置.

交叉熵损失 训练起草模型预测正确的下一个 token :

其中 是真实 token , 是草稿分布. 分布匹配损失 惩罚草稿分布和目标分布之间的总变差距离:

由于总变差距离是接受率的直接代理: 每步接受概率等于, 最小化 直接最大化了预期的接受率.

置信度损失 是一个二元交叉熵, 训练置信度头预测来自公式的软接受标签:
...
**隐层状态通信 (Hidden state communication)**: 在并行工作节点之间传输目标模型的全词汇表 logits () 会造成严重的带宽瓶颈. 作为替代, 临时缓存目标模型前向传播的激活值, 并且只通信语言建模(LM)头之前的隐层状态. LM 头的投影计算随后只在草稿模型所在的工作节点上, 针对被采样出的目标位置局部执行. 这将每个 token 的通信复杂度降低到, 其中 是隐层维度.

**基于锚点的序列打包 (Anchor-bounded sequence packing)**: 为了将草稿模型的计算成本与目标模型的上下文长度解耦, 从训练序列中采样固定数量的草稿锚点, 并将这些孤立的预测块打包成密集的训练批次.们通过 token 级别的注意力索引而非标准的二维掩码来管理这种打包. 这在多个独立的序列和锚点之间维持了精确的因果掩码, 避免了与标准填充相关的计算和内存开销.
...
: 平均每个生成 token 的墙上时钟延迟(wall-clock latency)

: 草稿模型生成 个候选 token 所需的时间

: 目标模型在单次前向传播中并行验证所有候选的时间

: 每个解码周期实际被接受的 token 数量(期望值)

相比标准自回归解码的单步延迟(即目标模型生成一个 token 的时间), 加速比为:

假设每个位置 的条件接受概率为(即在前 个 token 均被接受的条件下, 第 个 token 被接受的概率), 则:

在 i.i.d. 简化假设下(各位置接受率独立同分布为), 这简化为一个截断几何级数:

关键性质:

当(草稿几乎完美匹配目标)时,, 即所有候选均被接受

当 时,, 退化为标准自回归(每周期仅获得一个 bonus token)

 对 极其敏感: 从 0.7 提升到 0.9, 当 时, 从 2.94 跃升到 5.22
...
**半自回归(DSpark)**:(接近并行), 介于两者之间

杠杆3独立于前两者, 但在高并发场景下成为**决定性因素**: 即使 很高, 不加选择地验证长草稿块仍会因为占用系统批处理容量而降低全局吞吐量.

另一方面, 投机解码的加速效果对 batch size 敏感:

**Batch**=1(延迟优先)**: 目标模型严重 memory-bound, GPU 算力大量闲置. 投机解码通过将 个 token 打包为一次前向传播, 将算术强度从 提升到, 趋向 compute-bound, 加速最显著.

**大Batch(吞吐优先)**: GPU 已趋于 compute-bound, 额外验证 token 不再"免费". 此时每增加一个验证 token 的边际成本为正, 投机解码的"免费验证"假设失效. 这正是 DSpark 置信度调度的核心动机.

### 3.2 投机解码的数学模型

投机解码的接受率 与草稿分布 和目标分布 之间存在精确的数学关系:

其中全变差距离(Total Variation Distance)定义为:

**几何直觉**: 等于两个分布在概率空间中的"重叠面积". 当 时,, 完美接受; 当 和 的支撑集完全不重叠时,.

从最优传输(OT)的视角, 我们可以将"分布对齐"问题形式化为:

其中 是 Wasserstein-1 距离, 是所有边际为 和 的联合分布.

**与投机解码的联系**: 虽然投机解码直接使用 TV 距离(而非 Wasserstein 距离)来定义接受率, 但 OT 视角提供了更深层的理解:

**TV 距离是 OT 的特殊情况**: 当度量空间为离散的 时,.

**分布对齐的"运输代价"**: 草稿模型训练的本质是**最小化将 运输到 的代价**. 在投机解码中, 这等价于最大化接受率:
...
其中 是在计算预算 下可实现的草稿模型集合. 这不是一个标准的 OT 问题, 因为:

**模型容量约束**: 草稿模型的参数量/层数直接决定了其表达能力上界

**延迟约束**: 必须远小于, 否则投机解码无收益

**逐位置非独立**: 位置 的接受率依赖于前缀路径
...
 辅助对齐 mode(确保 argmax 正确)

权重 表明分布对齐比点预测更重要

这揭示了投机解码训练的**第一性原理**: 草稿模型的训练目标不应是"预测下一个正确 token", 而应是"使整个输出分布尽可能接近目标模型"(分布匹配目标).

另外在DSpark 的实验揭示了一个关键现象: 并行起草模型的接受率沿位置快速衰减.从 OT 视角, 这是因为**条件分布的运输代价随深度增加** DSpark 半自回归的 OT 解释为: 串行头注入的转移偏差 相当于在 OT 问题中引入了一个**条件运输映射**, 将并行主干输出的边缘分布 沿着实际采样路径进行"投影", 从而减少与目标条件分布 的运输距离.
...
: 硬件在 batch 下的每秒步数(throughput profile)

置信度调度(DSpark)核心思想: 让草稿模型自身评估每个位置的接受概率, 基于此动态决定验证长度.
置信度头设计

输入: 并行主干隐藏状态 + 前一 token 的马尔可夫嵌入

输出: 位置 在所有前序 token 被接受条件下的条件接受概率

监督信号:(解析接受率)
前缀存活概率
由链式法则:

 是草稿前缀 被完整接受的联合概率. DSpark 的动态调度: 将所有请求的候选 token 按 全局排序, 贪心准入直到系统吞吐量不再增加:
...
在高并发下, 每个验证 slot 具有**机会成本**: 它本可以用于服务另一个活动请求. 设系统有 个活动请求, 为请求 多验证一个 token 的边际收益和成本分别为:

预期增加的接受数

吞吐量下降

DSpark 的贪心算法本质上在做: 当 时停止准入.

调度策略的理论极限来看, 对于一个接受率为 的理想系统, 无论调度策略如何, 加速比的理论上界为:

当

**系统约束下的实际上界**: 在并发度 和 GPU 容量 的约束下:

当 较大时, 第二项成为瓶颈, 这正是调度策略需要解决的问题.
