...
将第 k 层的输入视为隐向量 $h_{k-1} \in \mathbb{R}^d$ (或 $h_{k-1} \in \mathbb{R}^{d \times 1}$), 网络的初始输入为 $h_0 \in \mathbb{R}^d$. 最初, $h_0$ 被复制 n 次, 形成初始的超隐状态矩阵

$$H_0 = (h_0 \ h_0 \ \dots \ h_0)^\intercal \in \mathbb{R}^{n \times d}$$
...
`h0` 的计算: 混音师根据 $A_m(H)$ 方案, 从 `n` 条音轨中混合出一个新的声音 $h'_0$, 送入一个效果器 (如混响, 对应 `T` 层).

`H'` 的计算: 同时, 混音师根据 $A_r(H)$ 方案, 对原始的 `n` 条音轨进行内部交叉混合, 准备好作为背景声.

**步骤 3 (核心计算)**: 效果器处理混合声 $h'_0$, 产生新的效果声 $\hat{H}$.

**步骤 4 (深度连接)**: 混音师根据 $B(H)$ 方案, 将效果声 $\hat{H}$ 混合回 `n` 条背景声 $H'$ 中, 形成最终输出的 `n` 条新音轨, 传递给下一个混音站.
...
$$x_{l+1} = \mathcal{H}_{l}^{\text{res}} x_l + (\text{残差部分})$$
...
$$x_L = \mathcal{H}_{\text{composite}} \ x_l + (\text{残差部分})$$

如下图所示, ResNet结构来自任意浅层 $l$ 的信息, 可以通过一个不含任何变换的、纯粹的加法路径, 直接传递到任意深层 $L$ 这条纯粹的加法路径就是恒等映射 (Identity Mapping).

![图片](assets/8d24805f727d.jpg)

而在HC的设计中, $\mathcal{H}_{i}^{\text{res}}$ 是一个**无约束的 (unconstrained)** 可学习矩阵. 这意味着它的元素可以是任意值, 它的性质 (如范数, 行列和) 是完全不可控的.矩阵连乘会产生指数级的放大或缩小效应.如果每个 $\mathcal{H}_{l}^{\text{res}}$ 的谱范数略大于1, 比如1.1, 那么经过 $k$ 层传播后, 复合矩阵的范数可能会增长到 $1.1^k$. 当网络很深时, $k$ 很大, 这会导致信号的**指数级爆炸**. 反之, 如果范数略小于1, 比如0.9, 就会导致信号的**指数级消失**.
...
具体来说, mHC利用**Sinkhorn-Knopp算法** (Sinkhorn and Knopp, 1967) 将 $\mathcal{H}_{l}^{\text{res}} x_l$ 进行熵投影到Birkhoff多面体 (Birkhoff polytope)上. 这个操作有效地将残差连接矩阵约束在由**双随机矩阵 (doubly stochastic matrices)** 构成的流形内. 由于这些矩阵的行和与列和都等于1, $\prod_{i=1}^{L-l} \mathcal{H}_{L-i}^{\text{res}}$ 这个操作起到了对输入特征进行凸组合 (convex combination)的作用. 这一特性促进了一种稳态的信号传播, 其中特征均值得以守恒, 并且信号范数被严格正则化, 有效地减轻了信号消失或爆炸的风险. 此外, 由于双随机矩阵对于矩阵乘法是闭合的 (closure), 复合映射 $\prod_{i=1}^{L-l} \mathcal{H}_{L-i}^{\text{res}}$ 保持了这种守恒属性. 因此, mHC有效地在任意深度之间维持了恒等映射的稳定性.
...
在HC的公式中, 第 $l$ 层的输入 $x_l \in \mathbb{R}^{1 \times C}$ 被扩展 $n$ 倍, 以构建一个隐矩阵 $x_l = (x_{l,0}^\top, \dots, x_{l,n-1}^\top)^\top \in \mathbb{R}^{n \times C}$, 这可以被看作是 $n$ 流的残差. 这个操作有效地加宽了残差流的宽度. 为了控制这个流的读出, 写入和更新过程, HC引入了三个可学习的线性映射: $\mathcal{H}_{l}^{\text{pre}}, \mathcal{H}_{l}^{\text{post}} \in \mathbb{R}^{1 \times n}$ 和 $\mathcal{H}_{l}^{\text{res}} \in \mathbb{R}^{n \times n}$.  这些映射修改了标准残差连接公式
...
| $\mathcal{H}_l^{\text{res}}$ | $\mathcal{H}_l^{\text{pre}}$ | $\mathcal{H}_l^{\text{post}}$ | 绝对损失差距 |
|:---:|:---:|:---:|:---:|
|  |  |  | 0.0 |
| ✅ |  |  | -0.022 |
| ✅ | ✅ |  | -0.025 |
...
基于复合映射的**行和的最大绝对值** $\max_i \sum_j |A_{ij}|$, 捕捉了前向传播中的最坏情况扩展.

基于**列和的最大绝对值** $\max_j \sum_i |A_{ij}|$, 对应于反向传播.
...
$$\left(\frac{\partial \mathcal{L}}{\partial x}\right)^\top = \left(\frac{\partial \mathcal{L}}{\partial y}\right)^\top A$$

根据不等式:

$$\left|\frac{\partial \mathcal{L}}{\partial x_j}\right| \le \sum_i \left|\frac{\partial \mathcal{L}}{\partial y_i}\right| |A_{ij}|$$
...
mHC是从ResNet中的恒等映射原理中汲取灵感, 核心前提是将残差映射 $\mathcal{H}_{l}^{\text{res}}$ 约束在一个特定的流形上. 原始的恒等映射通过强制 $\mathcal{H}_{l}^{\text{res}} = I$ (单位矩阵) 来确保稳定性, 但它从根本上排除了残差流内部的信息交换, 而这对于最大化多流架构的潜力至关重要. 因此作者建议将残差映射 $\mathcal{H}_{l}^{\text{res}}$ 投影到一个既能维持跨层信号传播稳定性, 又能促进残差流之间相互作用以保持模型表达能力的流形上.
...
也就是说, 如果 $X$ 是一个双随机矩阵, 那么存在 $\theta_1,...,\theta_k \ge 0, \sum_{i=1}^k \theta_i = 1$ 和置换矩阵 $P_1,...P_k$ 使得:

$$X = \theta_1P_1 + ... + \theta_kP_k$$

**这对mHC的流形约束有什么作用?**

对 $\mathcal{H}_{l}^{\text{res}}$ 操作, 本质是"加权平均的洗牌", 对输入 $x_l$ 应用 $\mathcal{H}_{l}^{\text{res}}$ 操作, 即计算 $\mathcal{H}_{l}^{\text{res}} x_l$, 等价于:

$$\mathcal{H}_{l}^{\text{res}} x_l = \left(\sum \theta_k P_k\right) x_l = \sum \theta_k (P_k x_l)$$
...
例如, $\mathcal{H}^{\text{res}}$ 的权重可以学习到"如果第1个流的某个特征很强, 并且第3个流的某个特征很弱, 那么在 $\mathcal{H}^{\text{res}}$ 中应该增强从第3流到第1流的连接强度". 这种全局上下文感知是原始HC所不具备的.
...
具体来说, 给定一个正矩阵 $M^{(0)} = \exp(\tilde{\mathcal{H}}_{l}^{\text{res}})$ 作为起点, 归一化迭代过程如下:
...
$\mathcal{H}_{l}^{\text{pre}} = \sigma(\tilde{\mathcal{H}}_{l}^{\text{pre}})$: 使用Sigmoid函数. 结果是一个元素在 (0, 1) 之间的向量. 这施加了**非负性约束**, 并且将所有权重归一化到一个小范围内. 这意味着从多流到单流的聚合是一个加权平均(如果后续再做归一化)或加权求和, 避免了负权重导致的信号抵消.

$\mathcal{H}_{l}^{\text{post}} = 2\sigma(\tilde{\mathcal{H}}_{l}^{\text{post}})$: 同样使用Sigmoid, 但乘以2, 使得元素范围在 (0, 2) 之间. 这同样施加了非负性, 但在将特征写回时适度放大信号. 为什么是2? 这可能是一个经验性的选择, 这种局部的, 单步的放大是可控的, 不像 $\tilde{\mathcal{H}}_{l}^{\text{res}}$ 的连乘那样会产生指数爆炸. 适度的放大可能有助于学习得更快.
...
给定一个所有元素都为正的方阵 $A$, 是否存在两个对角矩阵 $D_1$ 和 $D_2$, 使得 $B = D_1 A D_2$ 是一个双随机矩阵? 如果存在, 如何找到它们?

这里的核心挑战是如何通过简单的缩放操作, 同时满足"行和为1"和"列和为1"这两个看似相互冲突的约束. 如果你对所有行进行归一化, 行和就都变成1了, 但此时列和可能又乱了; 反之亦然.

Sinkhorn和Knopp证明了, 对于严格为正的矩阵 $A$, 这样的对角矩阵 $D_1, D_2$ 不仅存在, 而且是唯一的(在某个缩放因子内). 他们还提供了一个非常简单的迭代算法来找到这个结果, 这就是Sinkhorn-Knopp算法.

Sinkhorn-Knopp算法的核心思想非常直观: **既然不能一步到位, 那就轮流来**. 它通过一个迭代过程, 交替地满足行约束和列约束.

假设我们有一个 $n \times n$ 的方阵 $M$, 我们想把它变成一个双随机矩阵.

算法步骤:

**预处理**: 确保输入矩阵 $M$ 的所有元素都是正数. 在mHC中, 这一步是通过取指数完成的: $M \leftarrow \exp(\tilde{\mathcal{H}}_{l}^{\text{res}})$. 如果矩阵中已有0, 可能会导致问题, mHC通过`exp`操作可以完美避开.

**迭代**: 重复以下两个步骤, 直到收敛或达到最大迭代次数 $t_{\max}$:

a. `行归一化` : 将矩阵的每一行都除以该行的元素之和.

$$M \leftarrow M \oslash (M \mathbf{1}_{n \times 1} \mathbf{1}_{1 \times n}^\top)$$

这里 $\oslash$ 表示逐元素除法, $\mathbf{1}_{n \times 1}$ 是一个全1的列向量. 这个操作的效果是 $M_{ij} \leftarrow \frac{M_{ij}}{\sum_{k=1}^n M_{ik}}$

操作后, 矩阵的**所有行和都等于1**, 但列和可能不等于1.

b. `列归一化`: 将矩阵的每一列都除以该列的元素之和.

$$M \leftarrow M \oslash (\mathbf{1}_{n \times 1} \mathbf{1}_{1 \times n}^\top M)$$

这个操作的效果是 $M_{ij} \leftarrow \frac{M_{ij}}{\sum_{k=1}^n M_{kj}}$

操作后, 矩阵的**所有列和都等于1**, 但之前好不容易归一化的行和可能又被破坏了, 不再严格等于1.

**结束 (Termination)**: 当矩阵 $M$ 的变化非常小, 或者达到预设的迭代次数 (如mHC中的20次) 时, 算法结束. 最终得到的矩阵 就是一个(近似的)双随机矩阵.

> **与最优传输(Optimal Transport)的联系**
>
> **最优传输问题**: 想象你有两堆沙子(两个概率分布 $p$ 和 $q$), 你想以最小的"搬运成本"将第一堆沙子变成第二堆的样子. "搬运计划"就是一个矩阵 $P$, $P_{ij}$ 表示从第一堆沙子的位置 $i$ 搬运多少到第二堆沙子的位置 $j$.

离散形式的OT问题可以写成一个线性规划:

$$\min_{P \in \mathbb{R}^{n \times n}} \sum_{i,j} P_{ij} C_{ij}$$

$$s.t. \quad P_{ij} \ge 0, \ P \mathbf{1} = p, \ P^\top \mathbf{1} = q$$

其中 $C$ 是成本矩阵, $C_{ij}$ 是从位置 $i$ 搬到 $j$ 的单位成本. $p,q$ 是两个概率分布(向量). 在mHC中, 我们想将一个矩阵 $\exp(\tilde{\mathcal{H}})$ 投影到双随机流形上. 这相当于求解一个特殊的最优传输问题:

目标分布 $p$ 和 $q$ 都是均匀分布, 即 $p_i = q_j = 1/n$.

成本矩阵 $C$ 就是 $-\tilde{\mathcal{H}}$. (最大化 $\text{Tr}(\tilde{\mathcal{H}}^\top P)$ 等价于最小化 $\text{Tr}((-\tilde{\mathcal{H}})^\top P)$).

这个问题计算复杂度很高. 为了解决它, Marco Cuturi 在2013年提出了一个绝妙的想法: 在目标函数中加入一个**熵正则化项**:

$$\min_{P} \sum_{i,j} P_{ij} C_{ij} - \frac{1}{\lambda} H(P)$$

其中 $H(P) = -\sum_{i,j} P_{ij} \log P_{ij}$ 是矩阵 $P$ 的熵.这个问题有一个非常简单的解. 通过拉格朗日乘子法可以证明, 最优解的形式是: $P^* = \text{diag}(u) K \text{diag}(v)$ 其中 $K = \exp(-\lambda C)$, $u,v$ 是两个需要求解的对角缩放向量.

而求解 $u, v$ 的过程, 恰好就是Sinkhorn-Knopp算法! 具体来说, 就是迭代地更新 $u$ 和 $v$:

$$u \leftarrow p \oslash (K v)$$

$$v \leftarrow q \oslash (K^\top u)$$

这个过程等价于对矩阵 $K$ 交替地进行行归一化和列归一化, 使其行和等于分布 $p$, 列和等于分布 $q$.

因此, mHC中使用的Sinkhorn-Knopp算法, 实际上是在高效地求解一个熵正则化的最优传输问题, 其目的是找到一个与原始生成矩阵 $\tilde{\mathcal{H}}$ "最相似" 的双随机矩阵.
...
这一节阐述了在工程实现上的细节, 通过严格的优化, 作者在大型模型中实现了mHC (当 $n=4$ 时), 其边际训练开销仅为6.7%.

#### 4.3.1 算子融合

首先观察到mHC中的RMSNorm在处理高维隐藏状态 $\hat{x}_l \in \mathbb{R}^{1 \times nC}$ 时会带来显著的延迟, 因此将除以范数的操作重新排序到矩阵乘法之后. 这种优化在保持数学等价性的同时提高了效率.

![图片](assets/bf2ac8031c7d.png)

基于公式(10)到(13)中详述的输入和参数, 实现了三个专门的mHC算子来计算 $\mathcal{H}_{l}^{\text{pre}}, \mathcal{H}_{l}^{\text{post}}$ 和 $\mathcal{H}_{l}^{\text{res}}$. 在这些算子中, 偏置和线性投影被合并为 $b_l$ 和 $\varphi_l$, RMSNorm的权重也被吸收到 $\hat{x}_l$ 中.

公式(14)到(15): 作者开发了一个统一的算子, 它融合了对 $\hat{x}_l$ 的两次扫描(一次用于矩阵乘法, 一次用于计算范数), 利用矩阵乘法单元来最大化内存带宽利用率. 反向传播(包含两次矩阵乘法)同样被合并成一个单一算子, 消除了对 $\hat{x}_l$ 的重复加载. 这两个算子都具有一个精细调优的流水线(加载, 类型转换, 计算, 存储)来高效地处理混合精度.
...
$$r = \sqrt{\frac{1}{nC} \sum (\hat{x}_l)_i^2 + \epsilon}$$

这个过程需要完整地读取一遍高维向量 $\hat{x}_l$.

然后处理归一化并应用可学习的增益: 将输入向量 $x$ 的每个元素都除以这个 RMS 值, 再乘以一个可学习的增益 (gain) 参数 $g$

$$\hat{x}'_l = \frac{\hat{x}_l}{r} \odot g$$

注: 原始公式中的缩放因子 $g$ 可预先乘入后续权重矩阵 $\varphi_l$, 即实际计算的是 $(\frac{x}{r}) (g \varphi_l)$.

这个过程需要再次完整地读取向量 $\hat{x}_l$ (用于做除法), 并将结果 $\hat{x}'_l$ 写入内存.因此整个流程中, 对高维向量 $\hat{x}_l$ 进行了至少两次完整的内存读取. 对于非常大的 $nC$, $\hat{x}_l$ 是一个很大的张量. 这种重复的, Memory Bound的读写操作会成为性能瓶颈, 导致显著的延迟.

作者提出的优化是重新排序操作. 利用线性运算的性质.原始计算可以写作:

$$(\frac{\hat{x}_l}{r}) \cdot \varphi_l$$

由于除法 $1/r$ 是一个标量, 我们可以交换运算顺序:

$$(\frac{\hat{x}_l}{r}) \cdot \varphi_l = \frac{1}{r} (\hat{x}_l \cdot \varphi_l)$$

计算未归一化的矩阵乘法结果:

$$\text{IntermediateResult} = \hat{x}_l \cdot \varphi_l$$

同时计算 $\hat{x}_l$ 的L2范数所需的分母

$$r = \sqrt{\frac{1}{nC} \sum (\hat{x}_l)_i^2 + \epsilon}$$

在一次读取 $\hat{x}_l$ 的过程中, 同时完成矩阵乘法所需的计算和范数计算所需的平方和累加,这样, 对 $\hat{x}_l$ 的访存从两次减少到了一次.

最后, 计算最终结果:

$$\text{Result} = \frac{1}{r} \cdot \text{IntermediateResult}$$

这一步是在较小的 IntermediateResult 张量上进行的逐元素乘法, 其开销小于在原始高维向量 $\hat{x}_l$ 上进行的操作.
...
```python
def sinkhorn_knopp_naive(matrix, num_iters=20):
    matrix = torch.exp(matrix)  # Kernel 1: exp
    for _ in range(num_iters):
        # Kernel 2: Column normalization
        col_sum = torch.sum(matrix, dim=0, keepdim=True)
        matrix = matrix / col_sum
        # Kernel 3: Row normalization
...
>
> 我们需要计算损失 $\mathcal{L}$ 对Sinkhorn-Knopp算法的输入 $\tilde{\mathcal{H}}_{l}^{\text{res}}$ 的梯度, 即 $\frac{\partial \mathcal{L}}{\partial \tilde{\mathcal{H}}_{l}^{\text{res}}}$.

标准的自动微分框架(如PyTorch)在处理一个迭代过程时, 为了计算反向梯度, 通常会保存下前向传播过程中的所有中间变量. 在这个例子中, 它会保存下每一次迭代后的矩阵 $M^{(0)}, M^{(1)}, \dots, M^{(19)}$. 然后, 从后往前, 依次计算每一小步的梯度.

$$M^{(20)} \xrightarrow{\text{grad}} M'^{(19)} \xrightarrow{\text{grad}} M^{(19)} \xrightarrow{\text{grad}} \dots \xrightarrow{\text{grad}} M^{(0)} \xrightarrow{\text{grad}} \tilde{\mathcal{H}}_{l}^{\text{res}}$$
...
每次迭代前的矩阵: $M^{(0)}, M^{(1)}, \dots, M^{(T-1)}$.

列归一化后的中间矩阵: $M^{(0)'}, M^{(1)'}, \dots, M^{(T-1)'}$.

每次归一化时使用的分母:

列和: $c^{(t-1)}_j = \sum_i M^{(t-1)}_{ij}$ for $t=1, \dots, T$.

行和: $r^{(t)}_i = \sum_j M^{(t-1)'}_{ij}$ for $t=1, \dots, T$.

对于 $n=4, T=20$, 这些变量所需的SMEM空间是可控的, 完全可以放在片上.

反向传播的可以应用链式法则. 我们从最后一步开始, 逐层往回计算梯度. 我们用 $\bar{X}$ 表示 $\frac{\partial \mathcal{L}}{\partial X}$.

**1. 反向传播通过最后的 $T_r$ (RowNorm)**

前向: $M^{(T)}_{ij} = \frac{M^{(T-1)'}_{ij}}{r^{(T)}_i}$, 其中 $r^{(T)}_i = \sum_k M^{(T-1)'}_{ik}$.

已知: $\bar{M}^{(T)}$ (就是上游梯度 $\bar{H}$).

目标: 计算 $\bar{M}^{(T-1)'}$.

根据链式法则, 对 $M^{(T-1)'}_{ab}$ 求导:

$$\bar{M}^{(T-1)'}_{ab} = \frac{\partial \mathcal{L}}{\partial M^{(T-1)'}_{ab}} = \sum_{i,j} \frac{\partial \mathcal{L}}{\partial M^{(T)}_{ij}} \frac{\partial M^{(T)}_{ij}}{\partial M^{(T-1)'}_{ab}} = \sum_{i,j} \bar{M}^{(T)}_{ij} \frac{\partial M^{(T)}_{ij}}{\partial M^{(T-1)'}_{ab}}$$

我们分析 $\frac{\partial M^{(T)}_{ij}}{\partial M^{(T-1)'}_{ab}}$:

当 $i \ne a$ 时, 那么 $M^{(T)}_{ij}$ 与 $M^{(T-1)'}_{ab}$ 无关, 导数为0.

当 $i = a$ 时, 那么

$$M^{(T)}_{aj} = \frac{M^{(T-1)'}_{aj}}{\sum_k M^{(T-1)'}_{ak}}$$

当 $j=b$ 时 (对分子求导):

$$\frac{\partial M^{(T)}_{aj}}{\partial M^{(T-1)'}_{ab}} = \frac{1}{r^{(T)}_a}$$

当 $j \ne b$ 时 (对分母求导):

$$\frac{\partial M^{(T)}_{aj}}{\partial M^{(T-1)'}_{ab}} = -\frac{M^{(T-1)'}_{aj}}{(r^{(T)}_a)^2} = -\frac{M^{(T)}_{aj}}{r^{(T)}_a}$$

代入求和公式, 对于固定的 $(a, b)$:

$$\bar{M}^{(T-1)'}_{ab} = \bar{M}^{(T)}_{ab} \frac{1}{r^{(T)}_a} + \sum_{j \ne b} \bar{M}^{(T)}_{aj} \left(-\frac{M^{(T)}_{aj}}{r^{(T)}_a}\right)$$

这个式子可以进一步整理:

$$\bar{M}^{(T-1)'}_{ab} = \frac{1}{r^{(T)}_a} \left( \bar{M}^{(T)}_{ab} - \left(\sum_{j} \bar{M}^{(T)}_{aj} M^{(T)}_{aj}\right) + \bar{M}^{(T)}_{ab} M^{(T)}_{ab} \right)$$

注意到 $\sum_j \bar{M}^{(T)}_{aj} M^{(T)}_{aj}$ 是向量 grad 和向量 output 的点积.

**2. 反向传播通过最后的 $T_c$ (ColNorm)**

这一步与上一步完全对称, 只是从行操作变成了列操作.

前向: $M^{(T-1)'}_{ij} = \frac{M^{(T-1)}_{ij}}{c^{(T-1)}_j}$, 其中 $c^{(T-1)}_j = \sum_k M^{(T-1)}_{kj}$.
...
$$\bar{M}^{(T-1)}_{:,b} = \frac{1}{c^{(T-1)}_b} \left(\bar{M}^{(T-1)'}_{:,b} - (\bar{M}^{(T-1)'}_{:,b} \cdot M^{(T-1)'}_{:,b}) \cdot \mathbf{1}\right)$$

使用上述算子派生出的系数, 引入了两个额外的算子来应用这些映射: 一个用于 $\mathcal{F}_{\text{pre}}$ (应用pre映射), 另一个用于 $\mathcal{F}_{\text{post,res}}$ (应用post和res映射).通过将 $\mathcal{H}_{l}^{\text{post}}$ 和 $\mathcal{H}_{l}^{\text{res}}$ 的应用与残差合并操作相融合, 对于这个算子, 将读取的元素数量从 $(3n+1)C$ 减少到 $(n+1)C$, 将写入的元素数量从 $3nC$ 减少到 $nC$.

值得注意的是大部分算子(不包括公式(14)到(15))都采用TileLang高效的实现, $C$ 简化了具有复杂计算过程的算子的实现, 并允许以最小的工程量充分利用内存带宽. 那么是不是DeepSeek TileLang vs OAI Triton了呢?

#### 4.3.2 Recomputing

mHC的核心是将残差流从 $C$ 维扩展到 $nC$ 维. 在标准的训练流程中, 为了反向传播, 前向传播路径上的所有激活值都需要被保存下来. 这意味着每一层的输入 $x_l \in \mathbb{R}^{n \times C}$ 都需要存储. 对于一个有 $L$ 层的网络, 当 $n=4$ 时, 这部分的内存开销是标准Transformer的4倍, 这对于大模型来说是不可接受的.
...
前向传播: 对于一个由 $L_r$ 层构成的块, 只保存块的第一个输入 $x_{l_0}$. 在块内部, $x_{l_0+1}, x_{l_0+2}, \dots$ 都不保存.

反向传播: 当梯度反传到这个块的末端时, 为了计算对块内权重的梯度, 需要块内每一层的输入. 此时, 从保存的 $x_{l_0}$ 开始, 重新执行一次这个块的前向计算(只计算mHC部分和 $\mathcal{F}$ 的输入, 不需要完整计算), 从而得到所有需要的中间激活值, 然后再执行真正的反向计算.

在流水线并行中, 模型被切分到不同的GPU上, 每个GPU构成一个stage. 激活值需要在阶段之间传递. 重计算不能跨越这个边界, 因为下一个阶段的GPU没有上一个阶段的权重和输入, 无法进行重计算. 作者观察到理论最优值 $L_r^*$ 通常与每个流水线阶段的层数相符, 因此选择将重计算的边界与流水线阶段同步.
...
值得注意的是, 为了防止阻塞通信流, 作者在一个专用的高优先级计算流上执行MLP(即FFN)层的 $\mathcal{F}_{\text{post,res}}$ 算子. 另一方面还进一步避免在Attention层中的长耗时操作上使用持久化算子, 从而防止长时间的停顿. 这种设计使得被Overlap的Attention计算可以被抢占, 从而允许灵活的调度, 同时保持计算设备处理单元的高利用率.此外, 重计算过程与流水线通信依赖是解耦的, 因为每个阶段的初始激活值 $x_{l_0}$ 已经被缓存在本地了.
...
为了公平比较, HC和mHC都使用了相同的扩展率 $n=4$. 这确保了变量的控制, 使得性能差异能主要归因于"约束"的有无, 而不是扩展率的不同.
...
从Optimal Transport的视角, 在mHC中, 我们想将一个矩阵 $\exp(\tilde{\mathcal{H}})$ 投影到双随机流形上. 这相当于求解一个特殊的最优传输问题:
