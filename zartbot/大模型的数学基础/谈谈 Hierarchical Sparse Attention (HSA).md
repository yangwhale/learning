...
伴随着Coding/Agent任务context长度越来越长, 对于一个Dense Attention运算复杂度为 $\mathcal O(N^2)$, 同时softmax对于SFU的开销也是很大的. 在B200中SFU的算力相对于H200并没有太大的提升, 直到B300才通过砍其它高精度的算力把它提升上去.
...
在DSA中. lightning indexer,计算 Query token $h_t \in \mathbb{R}^d$ 与其之前的token $h_s \in \mathbb{R}^d$ 之间的索引得分 $I_{t,s}$, 这个得分决定了哪些token将被该查询token选中:
...
范畴论中, 一个小范畴 $C$ 的**神经(nerve) $N(C)$ 是一个由 $C$ 的对象和态射构造出的单纯集. 这个单纯集的几何实现是一个拓扑空间, 称为范畴 $X$ 的分类空间**. 这些紧密相关的对象可以利用代数拓扑学(最常用的是同伦论)的方法, 为一些我们熟悉且有用的范畴提供信息.

范畴的神经常被用来构造模空间(Moduli spaces)的拓扑版本. 如果 $C$ 是 $X$ 的一个对象, 它的模空间应该以某种方式编码所有同构于 $X$ 的对象, 并记录这些对象之间的各种同构关系. 这可能会变得相当复杂, 特别是当对象有很多非恒等自同构时. 神经提供了一种组合的方式来组织这些数据. 由于单纯集有很好的同伦论, 我们可以提出关于不同同伦群 $\pi_n(N(C))$ 意义的问题. 我们希望这些问题的答案能为原始范畴 $C$ 或相关范畴提供有趣的信息.
...
$token X$ 到 $token Y$ 的attention为 $f$, $token Y$ 到 $token Z$ 的attention为 $g$. 而 $token X$ 到 $token Z$ 的attention可以看作 $f,g$ 的复合 $ g \circ f$. 对于由 $N$ 个Token和它的Attention构建的一张态射图上来看, 我们能否进行某种神经构造,避免任意两个token之间的attn运算来构造?

令 $C$ 为一个小范畴. $N(C)$ 的每个**0-单纯形**对应 $C$ 中的一个对象. $N(C)$ 的每个**1-单纯形**对应 $C$ 中的一个态射 $f: x \to y$, 即任意一个token $f: x \to y$ 构成一个0-单纯形, 它们之间的Dense Attn作为态射, 构成一个1-单纯形. 如上所示三个token $x \to y \to z$, 现在假设有可复合的态射 $g: y \to z$ 和 $f: x \to y$. 那么我们有它们的复合 $gf: x \to z$. 这个图暗示了我们的做法: 为这个交换三角添加一个**2-单纯形**. $N(C)$ 的每个2-单纯形都来自于这样一对可复合的态射.

**一般情况:** $N(C)_k$ (k-单纯形的集合) 由 $C$ 中 $k$ 个可复合态射组成的序列构成:

$$
A_0 \xrightarrow{f_1} A_1 \xrightarrow{f_2} A_2 \to \cdots \to A_{k-1} \xrightarrow{f_k} A_k
$$
...
$$
A_0 \to \cdots \to A_{i-1} \to A_i \to A_{i+1} \to \cdots \to A_k
$$
...
**函子式定义:** 单纯集也可以被看作从 $\Delta^{op}$ 到 $Set$ (集合范畴)的函子, 其中 $\Delta$ 是由全序有限集 $[n]=\{0,1,...,n\}$ 和保序映射构成的范畴. 我们可以将范畴 $C$ 的神经描述为函子 $\Delta^{op} \to \textbf{Set}$:
...
另一部分的想法来自于Universal Transformer.将输入序列 (长度为m, 维度为 $d$) 的初始词嵌入矩阵 $H^0 \in \mathbb R^{m \times d}$ 通过 $T$ 步迭代, 得到成最终的表征矩阵 $H^T \in \mathbb R^{m \times d}$ .

在每个递归步骤 $t \in [1,T]$, 对所有 $m$ 个位置的表征 $H^{t-1}$ 进行并行更新, 得到 $H^t$. 其更新公式如下:

$$
A^t = \text{LayerNorm}((H^{t-1} + P^t) + \text{MultiHeadSelfAttention}(H^{t-1} + P^t))
$$

$H^{t-1}$ 是上一步的表征. 同时加入了 $P^t$, 即位置和时间步编码 (Position and Time-step Embeddings).

类似DSA中的Indexer进行计算, 然后首先取T $\text{topK}=Seq\_len / \alpha$, $\alpha$ 是一个递归的超参数, 可以取 [4, 8, 16...]等值. 然后对取出的K个token通过第二个indexer计算, 第二个indexer可以适当的提高计算的维度或数值精度. 再取 $1 / \alpha$ 个token. 迭代 $T$ 轮后(例如T=4), 最后按某个阈值进行过滤, 得到每个Token需要计算Attention的index-map. 然后根据这个index-map构建sparse attention.
