...
> 作者: zartbot  
> 日期: 2025年2月9日 17:35  
> 原文: http://mp.weixin.qq.com/s?__biz=MzUxNzQ5MTExNw==&mid=2247493157&idx=1&sn=51c0e27a347dd3fe1ed868d87f667897&chksm=f995f6e7cee27ff1a95d59aefe6bcf4117115343b301dddcc3ea7646dfeb61b582a90d507a16#rd
...
从最早的Transformer架构来看, Attention Block的计算量为 $\mathcal{O}(N^2 d)$, MLP Block的计算量为 $\mathcal{O}(N d^2)$. 针对模型规模扩大下的算法优化自然就盯着这两个block来做了. 例如针对Attention Block的MHA,DeepSeek MLA以及Stepfun MFA等. 很多的优化主要是前期针对长文本的优化, 这些内容后面再单独来写一篇吧.
...
因此, 一个很朴素的想法就是定义一个辅助的损失函数(Aux_Loss), 例如有 $S$ 个token, $E$ 个专家, 对于每个专家收到的token的方差之和定义为Loss

$$l_{aux} = \frac{1}{E}\sum_{e=1}^{E}\left(\frac{c_e}{S}\right)^2$$
...
$$g_{S,E}=softmax(wg\cdot x_S) \qquad m_E = \frac{1}{S}\sum_{s=1}^{S}g_{S,E}$$
...
在同等专家参数量和计算成本不变的前提下, 对专家进行更细粒度的分割, 这种更精细的专家分割使得激活专家的组合更加灵活且适应性更强. 例如将MoE中的专家FFN分割成 $m$ 个更小的专家, 然后FFN的inter-dim降低到原来的 $1/m$, 相同的计算成本下, 激活专家的数量增加为原来的 $m$ 倍. 例如原来 $C_N^K$ 种专家选择, 扩展后可以多达 $C_{mN}^{mK}$ 种选择. 例如原来 $C_{16}^2=120$ 种专家选择, 扩展 $m=4$ 后可以多达 $C_{64}^8 = 4,426,165,368$ 种选择, 这样的组合大幅度的提升了准确性和更有针对性的获取知识的能力.
...
为实现这一目标，除细粒度专家分割策略外，Deepseek进一步隔离了 $Ks$ 个专家作为共享专家。无论路由模块如何分配，每个token均被确定性地分配给这些共享专家。为保持恒定计算成本，其他路由专家中激活的专家数量将减少 $Ks$ 个。结合共享专家隔离策略后，完整的DeepSeekMoE架构如下

$$g_{i,t} = \begin{cases} s_{i,t} & s_{i,t} \in TopK(s_{j,t}\,|\,Ks+1 \le j \le mN,\ mK-Ks) \\ 0 & Otherwise \end{cases}$$

$$s_{i,t} = Softmax_i(u_t^{l\,T} e_i^l)$$

其中 $\sum_{i=1}^{Ks} FFN_i(u_t^l)$ 为共享专家, $\sum_{i=Ks+1}^{mN}(g_{i,t}FFN_i(u_t^l))$ 为mN个细粒度专家.
...
为了避免路由崩塌定义了专家级的负载均衡损失函数, 其中 $N'=mN-Ks$ 即路由专家数量, $K'=mK-Ks$ 表示激活的细粒度专家数量, $c_i$ 为专家需要处理的Token数目. $\alpha_1$ 为超参数.

$$\mathcal{L}_{ExpBalance} = \alpha_1 \sum_{i=1}^{N'} f_i P_i$$

$$f_i = \frac{N'}{K'T}\sum_{t=1}^{T}\mathbf{1}(Token\ t\ selects\ Expert\ i)$$

$$P_i = \frac{1}{T}\sum_{t=1}^{T}s_{i,t}$$

和Gshard中的辅助损失函数相比, 在计算 $f_i$ 时进行了一个修正 $N'/K'$, 因为按照 $N'$ 个专家选择 $K'$, 均匀分配的时候每个专家的概率为 $K'/N'$, 因此这里修正后整体的损失函数就和专家选择策略无关了.
4.3.2 设备级负载均衡损失

另一方面是进行设备级的负载均衡, 保证庄稼尽量能够平均的路由到多个设备, 使得计算负载相对均衡, 避免长尾影响. 因此将专家分为 $D$ 组 $\{\mathcal{E}_1, \mathcal{E}_2, \ldots, \mathcal{E}_D\}$, 每组专家部署在一个设备上. 损失计算如下

$$\mathcal{L}_{DevBalance} = \alpha_2 \sum_{i=1}^{D} f_i' P_i'$$
...
4.3.4 代码实现
HuggingFace上有一段DeepseekMoE的函数[2], 我们注意到MoEGate函数的实现和论文是有一些差异的, $T$ 的Token数目是针对一个batch内的所有token进行负载均衡损失计算的. 然后在16B的模型内并没有执行夸设备的Loss函数.
...
注: 渣B对于这个问题是持怀疑态度的, 主要原因还是Mellanox的网络设备的影响导致的, QP-Scale太大以及alltoall带来的incast而产生的通信Cost.这个问题解决了应该就不需要太多的限制了. 当然Deepseek有一些测试发现只要分组设备数量大到一定阈值($M > 3$)后和原来的TopK也没性能上的差异了.

#### 5.1 Device-Limited Routing

第一种是设备受限制的路由, 限制MoE相关的通信成本. 主要是在进行专家并行时, 被路由的专家将分布在多个设备上. 当专家数特别大时, 单个Batch内的Token通信将覆盖大量的设备, 导致EP的通信成本增高. 因此, 在DeepSeek-V2上增加了一个约束, 每个Token最多只能被路由到 $M$ 个设备上. 具体来说，对于每个 token，首先选择包含最高亲和力评分专家的 $M$ 个设备。然后，在这些 $M$ 个设备上的专家中执行 top-K 选择。在实践中，发现当 $M=3$ 时，设备受限的路由可以实现与无限制的 top-K 路由大致相当的良好性能。
...
虽然通过Device-Limit Routing可以解决通信域的问题, 减少Fanout的通信量, 但是接收设备侧还是可能出现集中到几个专家激活的情况,导致通信阻塞, 因此又增加了一个通信负载均衡损失, 其中 $D$ 表示设备数, $T$ 表示Token数, $M$ 表示选择的Group数量, $\mathcal{E}_i$ 表示第i个专家.

$$\mathcal{L}_{CommBalance} = \alpha_3 \sum_{i=1}^{D} f_i'' P_i''$$

$$f_i'' = \frac{D}{MT}\sum_{t=1}^{T}\mathbf{1}(Token\ t\ is\ sent\ to\ Device\ i)$$

$$P_i'' = \sum_{j\in\mathcal{E}_i} P_j$$

$f_i''$ 中增加了一个修正因子 $D/M$ 使得Loss不随着设备数和分组数产生变化.
...
| 模型 | 专家数(激活) | 共享专家 | expert_grp | grp_limit | dim | MOE inter_dim |
|---|---|---|---|---|---|---|
| 16B | 64(6) | 2 | - | - | 2048 | 1408 |
| 236B | 160(6) | 2 | 8 | 3 | 5120 | 1536 |
| 671B | 256(8) | 1 | 8 | 4 | 7168 | 2048 |
...
$$g'_{i,t} = \begin{cases} s_{i,t} & s_{i,t} + b_i \in TopK(s_{j,t}\,|\,1 \le j \le Nr,\ Kr) \\ 0 & Otherwise \end{cases}$$
...
$$f_i = \frac{Nr}{Kr\,T}\sum_{t=1}^{T}\mathbf{1}\left(s_{i,t}\in TopK(\{s_{j,t}\,|\,1\le j\le Nr\},\ Kr)\right)$$

$$s'_{i,t} = \frac{s_{i,t}}{\sum_{j=1}^{Nr}g_{j,t}} \qquad P_i = \frac{1}{T}\sum_{t=1}^{T}s'_{i,t}$$
