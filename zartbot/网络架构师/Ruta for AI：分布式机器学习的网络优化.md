...
> 作者: zartbot  
> 日期: 2020年9月20日 14:10  
> 原文: http://mp.weixin.qq.com/s?__biz=MzUxNzQ5MTExNw==&mid=2247484313&idx=4&sn=25add69041d44118c9da667957ebc93b&chksm=f996135bcee19a4ddef76c39dbc5106cd4be4b3c4ace9aa7631e27565b86dc6eee6f516ef7cd#rd
...
例如采用Top-K的方式对明显变化的参数进行更新，腾讯最近一次公布的2分31秒完成ImageNet**训练也使用过《[2分31秒，腾讯创造128卡训练ImageNet新纪录](https://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA==&mid=2652078137&idx=5&sn=b1efed59c9edd187b944cb2ead6071ae&scene=21#wechat_redirect)》
...
以往HDD**慢的时候，压力都在模型传输上，后期随着SSD逐渐改善并随着RoCE以及NVMe-over-fabric等技术消除了存储的瓶颈，而计算的瓶颈随着异构的各种NPU出现也逐渐消除，网络的瓶颈则是随着近几年数据中心的建设也逐渐地释放开，那么还有什么可以改进的地方呢？
...
然后阿里的做法是继续使用基于CLOS**架构的数据中心，然后对端口进行编号的方式构建一个层次化的拓扑结构：
...
为了解决Incast的问题大家也是费心了，例如阿里去年SIGCOMM的论文HPCC**便是使用Inband Telemetry的方式实时监控buffer，而Google今年SIGCOMM的Swift则是解耦交换机buffer和接收方buffer。另一方面，阿里好像最近还在招人做Multipath TCP(MPTCP)和MPQUIC，并使用MultiStream等方式想通过CLOS交换机集群ECMP**的方式降低Incast的影响。而Fungible则是说可以通过智能网卡调度避免：
