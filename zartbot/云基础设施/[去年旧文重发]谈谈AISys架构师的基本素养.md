# [去年旧文重发]谈谈AISys架构师的基本素养

> 作者: zartbot  
> 日期: 2025年1月28日 01:49  
> 原文: http://mp.weixin.qq.com/s?__biz=MzUxNzQ5MTExNw==&mid=2247493090&idx=1&sn=f23cac95c9e8e7486dbffa8f6aa99208&chksm=f995f520cee27c36f5cb817657d2b706252e913e9dda5f99e9363bcef59d0153a4b808e47b9b#rd

---

### TL;DR

昨夜NV的股价终于在三万亿上的破绽出现了

![图片](assets/b87f41f9db1c.png)

似乎盘后还有消息

![图片](assets/88f29ed578f3.png)

但是, 至少在我所在的那个领域, 2023年的时候NV过来说他们的Roadmap时, 我们都全部实现完了, 后面等专利可以公开的时候大家都能看到了. 芯片上的差距除了工艺外, 体系结构上中美的差距并不大. 至于ScaleUP也有很多布局了, 超越NVLink并不是太难的事情, 难的是缺少一份齐心协力的剑走偏锋. 大模型架构和硬件的协同上也有很多事情搞清楚了.

这篇文章是在半年前DeepSeek发布MLA之后, 然后工业界还在反复争论GPU ScaleUP互联标准的时候发的. 
...
半年之后来回味, 为什么这些创新没有出现在大厂?  正如这篇文章最后所讲
...
知识在各个领域间的脱节才是根本啊, 当然还有那一份与众不同的勇气, 可惜很多人跪久了膝盖软了...“不跟上怎么超越呢?” 几十年前也有这样的人, 什么都是“苏联老大哥是对的”...
...
争论背后的本质是新的计算范式的出现, 使得体系结构发生了更加紧耦合的重构,而组织结构的调整尚未跟上. 无论是NV内部NVLink党和Mellanox**党的争论,还是其他很多企业都面临相同的问题, 计算/网络/存储/服务器/芯片等多个团队如何紧耦合, 更进一步的扩展到上层的算子/并行策略/算法等多个团队的协同.
...
[《包处理的艺术(4)-低延迟智能网卡设计》](https://mp.weixin.qq.com/s?__biz=MzUxNzQ5MTExNw==&mid=2247485370&idx=1&sn=3b5590ccf58909f2d390df00bfb5d853&scene=21#wechat_redirect)

举一个在网计算真正有用的例子, 这是很多高频交易商做一些ETF套利**算法的时候常见的处理方式. 当然还有更多的内容断人财路就不说了.

![图片](assets/1bb31394d37d.png)
...
[《GPU架构演化史:从1980年图形渲染到Blackwell》 ](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=MzUxNzQ5MTExNw==&action=getalbum&album_id=2538479717163761664&scene=173&from_msgid=2247487954&from_itemidx=3&count=3&nolastread=1#wechat_redirect)

当然不光只有英伟达这样的GPGPU**架构, SIMT体系架构,其它家的例如TPU/Tenstorrent/华为AScend等, 每个架构师都是绝顶聪明的人,看清楚他们的取舍非常重要, 因此在谈论ScaleOut和ScaleUP这些互联之前, 对于这些GPGPU的架构和workload的变化需要有更多的了解, 然后对它片上互联结构的缺陷需要更加深入的了解. 例如今年早些时候对某个系统进行调优的时候, 就通过推测一些芯片架构找到了问题并精确的对它进行了定量的分析, 兄弟团队开发出了新的算法拿到了收益.
...
最近还看到Intel 前CEO Pat老先生谈论DeepSeek,  Computing Obeys the gas law, Engineering is about constraints, Open wins!

![图片](assets/f08ecf312fda.png)