...
在物理地址层面, 一个布局可以表示为一个线性地址表达式, 形式为 $\sum_i y_i s_i$, 其中 $y_i$ 表示第 $i$ 维的索引, $s_i$ 是该维度对整个线性内存地址贡献的步长 (stride). 给定一个布局 $L = s : d = (s_0, s_1, \dots, s_{n-1}) : (d_0, d_1, \dots, d_{n-1})$, TileLang 采用了一种受 TVM启发的设计, 引入了一个基于 `IterVar` (迭代变量) 的可组合, 可堆叠的布局函数抽象. 由于一个 `IterVar` 可以封装步长信息, 布局表达式可以被简化为关于 `IterVar` 的代数形式. 因此, 一个布局函数可以正式地表示为一个映射 $f : \mathbb{K}^n \to \mathbb{K}^m$, 其中 $f$ 编码了从高级索引到内存地址的转换.
...
此外, TileLang 引入了 `Layout` 抽象的一个扩展, 称为 **Fragment**. 与标准布局不同, `Fragment` 布局总是产生一个形式为 $f : \mathbb{K}^n \to \mathbb{K}^2$ 的输出, 其中两个输出维度分别代表 **线程在寄存器文件中的位置** 和 **在本地寄存器文件中的索引**. 例如, 在图 1 的 Kernel 中, 在线程块级别分配了一个寄存器文件 `C_local`. 然而, 由于 GPU 寄存器文件必须在块内线程之间划分, `Fragment` 布局提供了对这种划分方案的精确描述.
...
**BitsandBytes**: 专为支持如 $W_{NF4}A_{FP16}$ 等格式而设计, 并提供高效 Kernel.

**Marlin**: 针对 $W_{INT4}A_{FP16}$ 计算的高度优化 Kernel.
...
echo "set(USE_CUDA ON)" >> config.cmake # or echo "set(USE_ROCM ON)" >> config.cmake to enable ROCm runtime
cmake ..
make -j 10
```
...
extern "C" int call(half_t* __restrict__ A, half_t* __restrict__ B, half_t* __restrict__ C, cudaStream_t stream=cudaStreamDefault) {
 ...
 cuuint64_t A_desc_globalDim[2]= {
1024,1024}
;
 cuuint64_t A_desc_globalStride[2]= {
2,2048}
;
 cuuint32_t A_desc_boxDim[2]= {
32,128}
;
 cuuint32_t A_desc_elementStrides[2]= {
1,1}
;
 ...
 CUresult A_desc_result = CUTLASS_CUDA_DRIVER_WRAPPER_CALL(cuTensorMapEncodeTiled)(    &A_desc, A_desc_type, A_desc_tensorRank, A_desc_globalAddress, A_desc_globalDim, A_desc_globalStride + 1, A_desc_boxDim, A_desc_elementStrides, A_desc_interleave, A_desc_swizzle, A_desc_l2Promotion, A_desc_oobFill);
