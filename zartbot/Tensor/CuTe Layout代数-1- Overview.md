...
对于一个Tensor, 从数据结构上来看它是一个多维数组. 对于Tensor中某个元素的逻辑坐标(Logical coord)和实际内存中的地址构成一个一一对应的映射. 而这个映射结构的基本单元就是一个Layout. 实际上一个Layout就是一个Layout函数
...
对于行主序(row-major)的4x8矩阵, Stride是 (8, 1), 逻辑坐标  对应的内存偏移量为 

![图片](assets/4f9fc8eeb856.png)

对于列主序(col-major)的4x8矩阵, Stride是 (1, 4). 逻辑坐标  对应的内存偏移量为 

![图片](assets/9f97c286f323.png)

因此我们就可以得到两种Layout:

同时, 对于逻辑索引到实际物理地址的Offset函数 无论是哪种布局, 都是 Coord 和 Stride 的点积:
...
那么其中的 中的Stride又该如何表示呢? 当我们把中的第一个的的作为T的即T=(2,2):(1,4)$ 则可以构造出如下的Layout
...
`单同态(monomorphism)`:，如果对于所有的态射, 成立。类似于函数定义的单射(injective)
...
2.2.3 图
在范畴论中, 通常通过图表的方式表示对象和态射, 态射通常在图中用箭头表示. 每个箭  都具有以下属性:

源对象 (Source Object) 或 定义域 (Domain): 箭头出发的对象, 记作 

目标对象 (Target Object) 或 上域 (Codomain): 箭头指向的对象, 记作 

范畴论中的可复合性, 即对于和，存在一个复合(Composition)  可以通过一个图表表示
...
对象和一组态射: 存在一个特定的对象  和一组从它出发 (或指向它) 的态射, 它们满足某个图表的交换性.

泛性质 (A Universal Property): 对于任何满足类似条件的其他对象，存在**唯一的态射**使得相应的图交换.

#### 2.6.1  拉回 (Pullback)

给定一个由两个态射  和  构成的 **Span**, 即 . 它们的 **拉回** 是一个对象  和两个态射  和 , 它们共同满足以下两个条件:

**交换性 (Commutativity)**: 下面的图表是 **交换的 (commutative)**, 意味着从  到  的两条路径是等价的, 即

**泛性质 (Universal Property)**: 对于`任何`其他满足类似条件的对象  和态射   和  (即 ), 都`存在唯一`态射 , 使得下图中的所有三角形都交换, 即:  且 
...
**推出** 是拉回的**对偶 (dual)** 概念. 它回答了这样一个问题, 假设有两个态射  和   都从同一个源对象  出发. 我们如何能找到一个“最好的”目标对象 , 它既能被  映射, 也能被  映射, 并且这种映射方式“尊重”了它们共同的源头 ?

给定一个由两个态射  和   构成的 **Co-span**, 即 .

它们的 **推出** 是一个对象  和两个态射  和 , 它们共同满足以下两个条件:

**交换性 (Commutativity)**: 下面的图是 **交换的**, 意味着从  到  的两条路径是等价的: 

**泛性质 (Universal Property)**: 对于`任何`其他满足类似条件的对象  和态射 , , 都`存在唯一`态射 , 使得下图中的所有三角形都交换, 即:     且 
...
在1.3节我们已经介绍过了一些代数抽象, 例如传统的布局通常用一个 stride 元组来描述, 例如  用于  矩阵. 对于数据加载, 由于GPU自身的内存层次化结构, 我们需要Layout允许**嵌套的, 层次化的**布局. 比如, 一个  的 tile 可以被看作是  的 `Block x Thread` 布局, 并且进一步还可以嵌套.
...
因此, 我们考虑到一个最简单的Flatten的Layout, 即在Shape和Stride中都没有嵌套, 只用一维的数组  表示, 对于一个Layout  为了便于描述, 我们再引入**Mode**概念

**Size:** 定义, 它是Shape中各个dim的积.

**Length:** 定义的Length为

**Mode:**  对于 是一个length=1的Layout,我们将其称为的Mode.

然后对于Shape和Stride做如下约束.  首先, 我们在整数对  上定义一个序关系  如下:

注: 此处实际上是按照Stride从小到大对Layout的Mode进行排序.

**定义:** 我们说一个扁平布局(Flatten Layout)

是**易处理的 (tractable)**, 如果对于所有整数对 , 以下条件成立:

序关系  首先按 stride 排序, stride 相同再按 shape 排序. 这定义了一种从 "最快变化" (stride 最小) 到 "最慢变化" (stride 最大) 的维度排序. 条件  整除  的含义是: 如果维度  比维度  变化更快, 那么维度  这一整块在内存中的跨度 ( ) 必须能够无缝地铺满构成维度  的一个单位.

**Example:** 考虑一个行主序的  矩阵, 其布局为 shape=(M, N), stride=(N, 1). 即Layout
...
`态射`:  由一个有限点集 (finite pointed sets) 之间的映射  指定
...
我们称这样的态射  位于  之上, 并称  为一个元组态射 (tuple morphism).

对于条件1和2, 表示每个源维度要么被丢弃/广播(映射到 `*` ), 要么唯一地映射到一个目标维度. 不能多个源维度挤占同一个目标维度. 对于条件3()表示映射前后, 维度的"大小"不变. 这确保了布局变换只是"重排"和"广播", 而不是改变张量本身的数据量.

**定义:** 如果  是一个元组态射, 那么由  编码的布局是

其 Shape 是  的定义域, 其 Stride 由下式给出:

对于, Shape是一个定义域, 右侧是一个中间表示(IR). 即.  实际上对于同一个Layout可以用多个IR表示. 例如一个Layout=(4,5):(1,64), IR表示可以为(4,16,5), (4,16,5,7), (4,2,8,5)等... 只需要能够满足Tuple范畴条件即可.

![图片](assets/0429e355f73b.png)

对于这些IR表示中, 我们希望获得一个最简的表示, 即没有多余条目(例如没有  中的 7 ), 未被映射的条目合并(例如对比 , 中的16是中(2,8)合并的). 对于这个最简的表示, 我们可以称为  具有**标准形式 (standard form)**.

进一步假设布局和态射是**非退化的 (non-degenerate)**, 这对应于如下条件

Stride 计算公式:  这个公式是(colexicographic, colex)编码的体现. 态射的目标  定义了一个线性的内存空间, 其大小为 ,  元素  的偏移量是 .

当源维度  映射到  时,  就继承了这个偏移量作为其 stride. 如果  映射到 `*`, 它的 stride 就是 0, 这对应于广播 (broadcasting).

在高层次上, "non-degenerate" (非退化) 是一个约束条件, 用于消除布局表示中的冗余. 具体来说, 它处理的是大小为 1 的维度. 一个大小为 1 的维度在逻辑上是微不足道的, 因为它只包含一个元素(索引为 0), 沿着这个维度移动没有任何意义.

反例: L=(1,8):(8,1)中 这违反了非退化条件, 所以它是一个退化布局.  这个布局实际上描述的是一个 8 元素的向量. 逻辑坐标  (其中 j 从 0 到 7) 映射到的物理偏移是. 这个  的维度是多余的, 它只是给一个简单的一维向量强加了一个二维的外壳, 因此可以退化为一个一维的布局.
...
**扁平 (Flat):** 和  都是简单的整数元组, 形如  和 .

**易处理 (Tractable):** 对于所有 , 如果  (即  或 ( 且 )) 且 , 那么  必须整除 .

**非退化 (Non-degenerate):** 如果 , 那么 .
B. 标准形式的非退化元组态射 (Non-degenerate Tuple Morphism of Standard Form)
这是一个元组态射  (由映射  定义) 同时满足以下三个条件:

**非退化 (Non-degenerate):** 如果 , 那么 .

**标准形式 (Standard Form):** 这个条件比较复杂, 论文中给出了严谨定义, 我们可以将其直观地理解为两点:

**无冗余 1:** 余定义域 T 中不包含值为 `1` 的元素. 即  对所有  成立. 因为任何 `1` 都可以被合并到其他元素中而不改变编码的布局.

**最大化合并:** 余定义域 T 中所有未被箭头指向的元素 (即不在  的像集中的元素) 都被合并成一个单一的元素 (如果存在多个的话). 这样做是为了保证表示的唯一性. 比如, 如果  和  都没有被箭头指向, 它们应该被合并成一个新的元素 .

**元组态射 (Tuple Morphism):** 满足之前提到的基本规则.

这个定理是连接**应用世界 (布局)** 和 **数学世界 (态射)** 的关键桥梁.一一对应表示:

: 从一个布局  出发, 构造出态射 , 再从  编码回布局, 得到的一定是原来的 .

: 从一个标准形式的态射  出发, 编码出布局 , 再从  构造回态射, 得到的一定是原来的 .

这两个性质源于**标准形式**的唯一性和构造算法的确定性. 从  构造  的过程本质上是"分解" stride, 而从  计算  的过程是"合成" stride. 由于标准形式保证了分解和合成的唯一性, 这两个操作自然互为逆运算.
...
对于函数, 它的输入是一个多维逻辑坐标 , 其中 , 输出是一个一维线性索引 . 它定义了一种将多维坐标空间"铺平"成一维线段的标准方式. 对于Layout函数, 我们可以看作为三个步骤:

**逻辑坐标 -> 线性索引 (输入):** 用  将输入的逻辑坐标  转换为一个单一的线性索引 .

**线性索引 -> 线性索引 (核心映射):** 将输入空间的线性索引  映射到输出空间的线性索引 .

**线性索引 -> 物理偏移 (输出):** 输出的线性索引  实际上就是我们想要的物理内存偏移.
...
如果 S 是一个大小为 M 的元组, 那么 

如果 S 和 T 是大小分别为 M 和 N 的元组, 且  是一个元组态射, 那么它的实现  就是  的布局函数.

特别地, 这个结果提供了一个简单的的证明, 即`元组态射`的组合(Composition)与`Layout`的组合(Composition)是兼容的.

首先来看如何构造, 对于一个输入索引 :

**Decompose:** 使用逆余字典序同构, 将线性索引  分解为多维逻辑坐标 .

**Map:** 对每个坐标分量 , 查找其在态射 `f` (即  映射) 中的去向.

如果 , 那么这个坐标分量  贡献到了目标空间的第  个维度. 我们记 .

如果 , 那么这个坐标分量  被丢弃 (它不贡献到目标坐标).

**Recompose:** 我们现在有了一组目标坐标分量  (其中某些 `y` 可能未定义, 但这在 `colex` 中可以处理为 0). 使用目标空间的余字典序同构, 将这些分量重组为一个单一的输出索引:
...
这正是我们上面推导的  的结果, 所以,  的函数定义可以写成:

其中  是一个根据  重排坐标的函数.

这个函子的作用是连接两个范畴.  就是  的布局函数, 即

直接计算法 (传统方法): `shape` + `stride` -> .

函子实现法 (范畴论方法): `morphism f` -> .
...
再次使用本章定理, 一个函数的布局函数是 , 那么这个函数本身一定是 
...
 这个条件是关键, 它意味着维度  和  在逻辑上相邻, 并且在物理内存中也是完全连续的. 这是一个优化操作, 用于简化布局的表示, 减少逻辑维度的数量, 使其更接近于物理内存的线性本质.
...
合并操作的本质是**识别并消除布局中的冗余逻辑维度**. 在 GPU 编程中, 我们希望逻辑布局尽可能地贴近物理内存. 例如, 一个 `4x8` 的 tile, 如果它是列主序存储的, 其 shape=(4, 8), stride=(1, 4). 满足合并条件 (). 合并后得到 `shape=(32), stride=(1)`. 这清晰地表明, 这 32 个元素在内存中是连续的, 非常适合使用 `memcpy` 或向量化加载指令(如 `ld.global.v4.b32`) 来进行高效访问.
...
同样, 在范畴 **Tuple** 中也有补的类比. 我们可以通过包含那些**未被 *f* 击中**的条目来计算一个元组态射 *f* 的补 . 例如:
...
补操作的本质是**描述一个子集之外的剩余部分**.想象一个大的数据张量,  描述了其中一个 tile 的布局. 那么  就描述了 "所有其他数据" 组成的集合的布局. 这个操作对于实现 Partitioning 和 Tiling 至关重要. CuTe 中一个常见的模式是 make_tile(L, comp(L)), 它将一个大张量划分为  描述的 tile 部分和  描述的剩余部分. 程序员可以处理  部分, 然后在  上递归地进行下一次划分.

另外  和  拼接后是紧凑的. "紧凑"意味着布局函数是单射的, 即没有两个不同的逻辑坐标映射到同一个物理地址, 并且其像集(所有物理地址的集合)是连续的. 换句话说,  完整地、无重叠地、无空洞地覆盖了整个数据张量.

另外,在态射的世界里, 求补操作变得直观.  给定态射 , 它描述了  中的维度如何"挑选"  中的某些维度. 它的补  的定义域就是  中那些`未被挑选`的维度组成的元组, 而  的态射则将这些维度"挑选"出来.

这使得程序员或编译器设计者可以从更高层次思考数据划分问题, 只需考虑"我需要这部分", 剩下的部分可以由系统通过 `comp` 操作自动、正确地计算出来.

### 4.3 复合 (Composition)

如果 A 和 B 是布局, 那么**复合 (composition)** 是一个布局, 使得对于任何 , 我们有:

还有其他属性可以唯一地刻画布局 .

例如, 如果 A = (2, 2) : (5, 50) 且 B = (5, 2, 5, 2) : (1, 25, 5, 50), 那么 A 和 B 的复合是 (2, 2) : (25, 50).

如果  和  是元组态射, 且 `codomain(f) = domain(g)`, 那么我们可以复合  和  形成元组态射 . 例如, 下图的态射是可复合的:
...
**复合 **: 描述了从 (blockIdx, threadIdx) 这样的 "全局线程ID" 直接到它应该访问的全局内存数据的映射.

在态射的世界里, 复合就是将布局图首尾相连, 然后消除中间部分.

#### 4.4 一些展开

对于4.3中的复合条件 `codomain(f) = domain(g)` 在现实世界中, 这个条件几乎总是**不满足**的. 比如布局A的输出是 `(128, 128)` 的 tile, 而布局B的输入是 `(32, 8)` 的线程网格. `(128, 128)` 和 `(32, 8)` 根本不匹配. 实际上我们是否有可能在上细化一个态射以及在上细化一个态射, 然后使得 能够表示 ?
...
在前面的章节中, 我们为扁平布局 (flat layouts) 建立了一套理论, 并指出了其局限性: 即使两个布局 `A` 和 `B` 在逻辑上是可复合的, 它们对应的标准型态射  和  也很可能因为 codomain 和 domain 不匹配而无法在范畴中直接组合. 本章引入**嵌套布局 (Nested Layouts)** 来解决这个问题, 并基于此构建一个通用的**组合算法 (Composition Algorithm)**.
...
我们首先定义一些术语
profile
Profile是一个嵌套元组, 其每个条目都是符号 . 例如  和  都是Profile.

一个**嵌套元组 (Nested Tuple)** 由其**扁平化 (flattening)** (一个普通元组) 及其Profile  唯一确定. 在使用嵌套元组时, 写作  会很方便. 例如, 如果 , 我们可以写作  其中 

如果  是一个布局, 那么  和  需要有相同的Profile, 所以我们可以将一个通用布局写作

我们称布局

为  的**扁平化**. 关于扁平布局的大部分理论可以轻松地移植到嵌套情况.

**定义:** 如果一个布局  的扁平化  是易处理的, 我们就说  是**易处理的 (tractable)**.
...
我们说这样的态射  位于  之上，并将  称为嵌套元组态射。

如果  是嵌套元组态射，则由  编码的布局是布局

其形状是  的定义域, 其Stride由以下公式给出:
...
如果  和  分别是大小为  和  的嵌套元组, 且  是一个元组态射, 那么其实现  就是  的布局函数.

特别地, 这个定理可以轻松证明以下结果:

**定理:** 如果  和  是可复合的嵌套元组态射, 那么:
...
我们在嵌套元组态射上定义了一个**合并**操作 , 它与布局的合并兼容, 即

我们在嵌套元组态射上定义了一个**互补**操作 , 它与布局的互补兼容, 即如果  是一个标准型的单射嵌套元组态射, 则

我们定义了嵌套元组态射的**可除性 (divisibility)** 概念, 以及当  整除  时的**逻辑除**操作 . 此操作与布局的逻辑剖分兼容, 即

我们定义了嵌套元组态射的**乘积可容许性 (product admissibility)** 概念, 以及当  和  乘积可容许时的**逻辑乘积**操作 . 此操作与布局的逻辑乘积兼容, 即
...
现在我们已经将理论推广到嵌套情况, 我们可以解释我们的**复合算法**了. 该算法使用范畴论框架来计算易处理布局 `A` 和 `B` 的组合 . 我们的算法中使用了一些我们尚未讨论的重要构造, 即**共同细化 (mutual refinements)**, **拉回 (pullbacks)**, 和**前推 (pushforwards)**.

假设我们想计算布局  和  的复合. 由于  和  都是易处理的, 我们可以用元组态射来表示它们:

![图片](assets/f7528db42c0e.png)

这些态射是不可复合的, 因为  的codomain (6, 6) 不等于  的 domain . 这意味着我们不能直接使用态射  和  来计算复合 .

然而, 我们可以通过找到 `(6, 6)` 和 `(12, 3, 6)` 的一个**共同细化 (mutual refinement)** 来继续我们的计算, 如下图所示:

![图片](assets/fce6385359e0.png)

直观上, 这样一个共同细化是一种规范, 说明了我们如何以一种兼容的方式分解  的codomain和  的domain. 我们可以使用我们的共同细化将  和  转换为可复合的态射  和 .

对于  的情况, 我们的共同细化指示我们应该将第一个 `6` 分解为 `(2, 3)`, 并在  的codomain中包含一个额外的 `6`:

![图片](assets/501c9dab5d17.png)

严格来说, 从  构建  的过程是**拉回 (pullback)** 的一个实例.

对于  的情况, 我们的共同细化指示我们应该将 `12` 分解为 `(6, 2)`:

![图片](assets/9e1fd5373b24.png)

严格来说, 从  构建  的过程是**前推 (pushforward)** 的一个实例.

嵌套元组态射  和  是可复合的, 所以我们可以形成它们的复合:
...
对于Layout  令 , B为另一个Layout, 假设是可补(Comlementation)的, 且是可复合的(Composition), 则定义逻辑除法(Logical Divide)如下:
...
算子是一种代数结构, 它专门用来描述**带有多个输入的运算以及如何组合它们**. 一个典型的例子是函数组合: 假如有函数  和 , 我们可以把  的输出作为  的一个输入, 形成一个新的函数 . 算子就是对这类"组合规则"的形式化和公理化.

一个最简单的算子包含:

对每个 , 有一个  元操作的集合 .

一个组合规则  允许将一个  元操作插入到一个  元操作的第  个"输入槽"中, 得到一个  元操作.

满足结合律和单位元等公理.

### 6.2 将布局理论嵌入算子理论

考虑在`整除关系`下由正整数构成的`偏序集`:  当且仅当  整除 . 如同任何偏序集一样, 我们可以关联一个范畴, 其对象是集合中的元素, 且当且仅当  时存在一个态射 . 为简便起见, 我们也用  表示这个范畴.

现在, 将这个范畴视为在乘法运算下的一个对称幺半范畴 (symmetric monoidal category, SMC), 并应用**算子神经 (operadic nerve)** 来产生算子, 它配备了一个到有限点集范畴的函子.

我们有一个wide subcategory(保留全部对象, 但仅保留部分态射) , 它是有限点集上那些在基点之外是单射的映射构成的, 这个  是单个一元操作的算子. 然后, 将  拉回到  上, 其结果等同于排除了条件 2c(即如果  , 则) 的 **Tuple** 范畴的定义, 而施加条件 2c 则将 **Tuple** 定义为这个拉回的一个子范畴.

算子神经(operadic nerve )
它是类似于范畴论中的Nerve[3]构造, Nerve 用于将一个范畴转化为一个单纯集合 (Simplicial Set). 一个单纯集合 `X` 不是一个单独的集合, 而是一族集合  以及它们之间特定的*面映射 (face maps)* 和*退化映射 (degeneracy maps)*.

: 0-单纯形的集合 (可以想象成*顶点/点*).

: 1-单纯形的集合 (可以想象成*有向边/线段*).

: 2-单纯形的集合 (可以想象成*填充好的三角形*).

: 3-单纯形的集合 (可以想象成*填充好的四面体*).
...
算子 (Operad)处理的是*树状/多输入组合*. 一个算子操作有多个输入和一个输出, 就像一个函数 . 这些操作可以像树一样嵌套组合, 例如 . *算子神经* 正是将一种特定的代数结构, 即*对称幺半范畴 (Symmetric Monoidal Category, SMC)* "几何化"成一个算子.
对称幺半范畴 (SMC)
一个 SMC 是一个范畴 , 同时配备了一个"积"运算  , 它类似于乘法. 这个  运算满足:

*结合律:*  与   是"相同"的(在范畴论意义下是自然同构的).

*交换律:* 与  是"相同"的.

*单位元:* 存在一个对象 , 使得  与  是"相同"的.

对于Layout代数, 使用的 SMC 是 .
...
算子神经作用于一个 SMC, 如  时, 它生成的算子中的  元操作, 对应于用  运算将  个对象组合起来的所有可能方式. 这些"组合方式"可以用`树`来可视化. SMC 中的 `n` 个对象  ,用  运算将它们组合, 组合后的结果 .

算子神经将这种 "组合树"的结构几何化 了. 在它生成的算子范畴中: *对象*是原 SMC 中的对象 (例如, 正整数).  一个  元态射 (n-ary morphism)  对应于一个从  到  的态射. 在   的例子中, 这就是一个整除关系   整除 

进行这个构造的目的是为了*从最基本的数学原理出发, 构建出能够描述 CuTe 布局的范畴*.

*起点:* 作者从最简单的结构  出发. 这代表了布局 `shape` 维度可以被乘起来合并 (`coalesce`) 这一基本事实.

*应用算子神经:* 对 应用算子神经, 得到了算子 . 这个新生成的算子是一个非常丰富的结构, 它内在地包含了所有关于"将一串数字通过乘法组合起来"的规则.

 中的一个`n`元操作, 就对应于一个  维的`shape`元组 .

  中的组合, 就对应于将 `shape` 元组进行嵌套和合并. 例如, 将  和  组合成 .

*构造 Tuple范畴:* 这个结构过于庞大和通用. CuTe 的布局态射还有额外的约束, 比如"一个输入维度最多映射到一个输出维度" (单射性). 因此, 作者通过拉回 (pullback)的技巧, 从  中"裁剪"出了一个子结构, 这个子结构恰好就是前面定义的 *Tuple范畴*.

从这个角度来看, 我们又该如何整合 profiles 呢? profile本身构成一个(单色的, 对称的)算子, 其  元操作的集合由长度为  且对称群作用平凡的profile组成. 在算子神经下, 将此算子表示为 . 然后, 通过构造算子的拉回, 就可以考虑在任何对称幺半范畴  中带有标签的profile; 对于, 我们将得到的拉回表示为 . 于是, 考虑深度为1的profile, 我们同样得到 **Tuple** 是  在   上的拉回的一个子范畴 (实际上, 也是  自身的子范畴).

此外, 由于   同时包含了元组态射和细化 (因为整数乘法是幺半积), 它是一个合适的范畴可以在其中进行更复杂的构造. 具体来说, 正如我们在组合算法中出现的图中所看到的, 将因式分解后面跟着元组态射的过程, 视为其本身就构成了某个范畴中的态射.

范畴论中有一个标准的构造可以为我们做到这一点; 即, 我们可以在   中构造一个特定的Span范畴[4]. 在这个范畴中, 前向态射的类是wide subcategory **Tuple** 中的态射, 而后向态射的类 **Ref** 由那些在有限点集映射  上的cocartesian edges[5] 组成, 这些映射  满足:

 是active的: 如果 , 那么 .

 是满射的 (surjective).

当限制在  上时,  是非递减的 (non-decreasing).
...
在这里, 在这类映射上取余笛卡尔边的效果, 意味着我们考虑的是例如映射  其中 , 而不是  整除  的一般情况 . 也就是说, 我们得到的精确是细化(refinement).

注意, 为了让Span构造是良定义(well-defined)的, 我们需要检查在  中, **Tuple** 中的态射沿着 **Ref** 中的态射的拉回是可以形成的. 这一点可以被证明.
...
根据定义, **Span(Tuple, Ref)** 包含  和 **** (Ref 的反范畴) 作为子范畴. 然后我们可以将实现函子从 **Tuple** 扩展到 **FinSet**, 使其作用于整个 **Span(Tuple, Ref)** 范畴, 这样**细化 (refinements)** 就被映射到*逆余字典序同构 (inverse colexicographic isomorphisms)*.
...
作者的出发点是正整数集合, 配合"乘法"运算. 这构成了一个**幺半群 (monoid)**.通过 " 当且仅当 a 整除 b" 这个关系, 作者把它变成了一个**偏序集**, 进而是一个**范畴**. 将这个范畴看作一个**对称幺半范畴**, 这里的"幺半积"就是整数乘法 .

接下来**构造算子: 算子神经 (Operadic Nerve)**

这是一个标准的技术, 可以从任何对称幺半范畴 (如  ) 构建出一个算子. 我们得到的算子  直观上可以这样理解:

一个 `n` 元操作就是 `n` 个正整数的元组 .

组合规则就是将一个元组插入到另一个元组中, 并在底层通过整数乘法来合并 `shape` 值. 这恰恰就是布局中"细化"或"合并"操作的雏形.

然后是**拉回 (Pullback) 构造**, 作者并未直接使用这个算子, 而是通过**拉回**操作来施加约束, 从而精确地构造出 `Tuple` 范畴.`Tuple` 范畴中的态射  有一个关键属性: 非基点映射是**单射** (一个输入维度最多映射到一个输出维度). 作者找到了编码这个属性的算子 (只含一元操作的算子).

通过将   拉回到 上, 得到的新结构就自动满足了单射属性. 再加上  这个条件, 就重构了 `Tuple` 范畴. 这一步的意义在于: 它说明 `Tuple` 范畴不是一个凭空捏造的结构, 而是由两个更基本的结构 ("乘法组合规则" 和 "单射映射规则") 通过标准的范畴论构造 (拉回) 自然产生的.

最后**引入Profile** , Profile本身也构成一个算子 , 它的操作就是"如何将 `n` 个东西组合成一个嵌套结构". 通过再次使用拉回, 将   和   结合起来, 得到 . 这个新的算子同时编码了**数值的乘法组合**和**结构的嵌套组合**. `Nest` 范畴就生活在这个更丰富的世界里.

然后进一步引入**Span范畴**

在第五章, 为了复合  和 , 我们需要先找到一个"共同细化", 然后对  做 `pullback` , 对  做 `pushforward`. 这个过程看起来像是在范畴之外进行了一些"手工操作".

作者提出, 我们可以构造一个新的范畴 `Span(Tuple, Ref)`, 在这个范畴里, 态射本身就包含了"细化"的过程.

一个**Span** 是形如  的一对态射. 在 **Span(Tuple, Ref)** 中, 一个态射  被定义为一个Span, 其中:

 来自于 **Ref** 范畴, 代表**细化 (Refinement)** 操作 (例如, 将 `(36)` 细化为 `(6,6)`).

 来自于 **Tuple** 范畴, 代表我们之前定义的**元组态射**.

直观上, 这个新范畴中的一个态射就代表了 "先对输入进行一次细化, 然后再进行一次元组映射" 这一整个过程. 复合算法中  到  的变换, 就可以看作是在 `Span(Tuple, Ref)` 范畴中, 将  与一个代表细化的态射进行组合.

通过 `Span` 范畴, 第五章中为了组合  和  而进行的"适配"操作, 现在变成了在新范畴中进行一次标准的态射组合.
